...
void solve() {
        ...
        vector<vector<vector<int>>> dp(4, vector<vector<int>>(n+5, vector<int>(n*25 + 5, -INF)));
        dp[1][0][0] = 0;
        for (int i = 1; i <= n; i++) {
                for (int j = 0; j <= i; j++) {
                        for (int k = 0; k <= n*25 - v[i].se; k++) {
                                if (j > 0) {
                                        dp[2][j][k + v[i].se] = max(dp[2][j][k + v[i].se], dp[1][j - 1][k] + v[i].fi);
                                }
