...
int main()
{
    ...
    // code
    ll n, a, arr[5000], b[5000], ans = 1000000000000000000, bruh;
    cin >> n;
    for (int i = 0; i < n; i++)
    {
        cin >> arr[i];
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = i - 1; j >= 0; j--)
        {
            if (j == i - 1) b[j] += arr[j], bruh++;
            // else if (arr[j] > b[j + 1]) b[j] += arr[j], bruh++;
            else b[j] += (b[j+1]/arr[j] + 1)*arr[j], bruh += (b[j+1]/arr[j] + 1);
        }
        for (int j = i + 1; j < n; j++)
        {
            if (j == i + 1) b[j] += arr[j], bruh++;
