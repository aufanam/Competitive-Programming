...
void solve() {
        ...
        for (auto z : d) {
                buset.insert({z.fi, z.se});
                mapan[z.se] = z.fi;
        }
        int ans = 0;
        for (auto it = buset.rbegin(); it != buset.rend(); it++) {
                int z = it->fi;
                int x = it->se;
                for (auto ix = v[x].rbegin(); ix != v[x].rend(); ix++) {
                        int zx = ix->fi;
                        int fd = ix->se;
                        if (mapan[fd] > 0 && mapan[x] > 0 && zx > 0) {
                                mapan[fd]--;
                                mapan[x]--;
                                ans += zx;
                                v[x].erase({zx, fd});
