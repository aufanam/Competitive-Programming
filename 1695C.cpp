...
void solve() {
        ...
        vector<vector<int>> neg(n, vector<int>(m));
        pos[0][0] = (v[0][0] == 1);
        neg[0][0] = (v[0][0] == -1);
        for (int i = 1; i < n; i++) {
                pos[i][0] = pos[i-1][0] + (v[i][0] == 1);
                neg[i][0] = neg[i-1][0] + (v[i][0] == -1);
        }
        for (int i = 1; i < m; i++) {
                pos[0][i] = pos[0][i-1] + (v[0][i] == 1);
                neg[0][i] = neg[0][i-1] + (v[0][i] == -1);
        }
        for (int i = 1; i < n; i++) {
                for (int j = 1; j < m; j++) {
                        pos[i][j] = max(pos[i][j-1], pos[i-1][j]) + (v[i][j] == 1);
                        neg[i][j] = max(neg[i][j-1], neg[i-1][j]) + (v[i][j] == -1);
                }
        }
        int x = (n + m - 1)/2;
        if (pos[n-1][m-1] >= x && neg[n-1][m-1] >= x) {
                cout << "YES\n";
        } else {
                cout << "NO\n";
        }
