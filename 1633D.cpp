...
int n, k;
vector<int> a(N), b(N), need(N, N);
void rec() {
        need[1] = 0;
        for (int i = 1; i <= 1005; i++) {
                for (int x = 1; x <= i; x++) {
                        int j = i + i/x;
                        if (j < N) ckmin(need[j], need[i] + 1);
                }
        }
}
...
void solve() {
        ...
        vector<int> dp(N, 0);
        int sum = 0;
        for (int i = 1; i <= n; i++) sum += need[a[i]];
        ckmin(k, sum);
        for (int i = 1; i <= n; i++) {
                for (int j = k - need[a[i]]; j >= 0; j--) {
                        dp[j + need[a[i]]] = max(dp[j + need[a[i]]], dp[j] + b[i]);
                }
        }
