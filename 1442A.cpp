...
void solve() {
        ...
        vector<int> decp(n);
        decp[0] = 0;
        for (int i = 1; i < n; i++) decp[i] = max(decp[i-1], v[i-1] - v[i]);
        vector<int> decs(n);
        decs[n-1] = 0;
        for (int i = n - 2; i >= 0; i--) decs[i] = max(decs[i+1], v[i+1] - v[i]);
        vector<int> pref(n);
        pref[0] = v[0];
        for (int i = 1; i < n; i++) pref[i] = min(pref[i-1] - decp[i], v[i]);
        vector<int> suf(n);
        suf[n-1] = v[n-1];
        for (int i = n - 2; i >= 0; i--) suf[i] = min(suf[i+1] - decs[i], v[i]);
        for (int i = 0; i < n; i++) {
                if (v[i] > pref[i] + suf[i]) {
                        cout << "NO\n";
                        return;
