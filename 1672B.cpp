...
int main()
{
    ...
    while (t--)
    {
        ...
        string x = "";
        int total = 0;
        bool bull = false, bully = false, bullx = false;
        for (int i = 0; i < s.length(); i++)
        {
            if (s[i] == 'A') total++, bullx = true;
            else total--, bully = true;
            if (total < 0) bull = true;
        }
        if (!bully || !bullx) cout << "NO\n";
        else if (bull) cout << "NO\n";
        else if (s[s.length()-1] == 'A') cout << "NO\n";
        else cout << "YES\n";
