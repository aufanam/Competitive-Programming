...
void solve() {
        ...
        for (int i = 0; i < n; i++) {
                int t, a, b;
                cin >> t >> a >> b;
                if (a && b) liked.push_back(t);
                else if (a) alice.push_back(t);
                else if (b) bob.push_back(t);
        }
        if (liked.size() + alice.size() < k || liked.size() + bob.size() < k) {
                cout << "-1\n";
                return;
        }
        sort(all(liked));
        sort(all(alice));
        sort(all(bob));
        vector<int> sumboth(liked.size()+1);
        vector<int> anih(alice.size()+1);
        vector<int> bnih(bob.size()+1);
        sumboth[0] = 0;
        anih[0] = 0;
        bnih[0] = 0;
        int sum = 0;
        ...
        int ans = INT_MAX;
        for (int i = 0; i <= k; i++) {
                if (i <= alice.size() && i <= bob.size() && (k - i) <= liked.size()) {
                        int res = anih[i] + bnih[i] + sumboth[k-i];
                        ckmin(ans, res);
                }
        }
        cout << ans << '\n';
