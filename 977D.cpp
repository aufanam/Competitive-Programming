...
void solve() {
        ...
        vector<pair<bool, bool>> zx(n);
        for (int i = 0; i < n; i++) {
                int fd = v[i];
                bool b1, b2, b3, b4;
                b1 = b2 = b3 = b4 = false;
                if (buset.count(fd*3)) b1 = true;
                if (fd%2 == 0) {
                        if (buset.count(fd/2)) b2 = true;
                }
                if (buset.count(fd*2)) b3 = true;
                if (fd%3 == 0) {
                        if (buset.count(fd/3)) b4 = true;
                }
                if (b1 || b2) zx[i].fi = true;
                else zx[i].fi = false;
                if (b3 || b4) zx[i].se = true;
                else zx[i].se = false;
        }
        int aw;
        int ak;
        ...
        for (int i = 1; i < n; i++) {
                int fd = zz.back();
                if (buset.count(fd*2)) zz.push_back(fd*2);
                else if (fd%3 == 0) {
                        if (buset.count(fd/3)) zz.push_back(fd/3);
                }
        }
        print(zz);
        return;
