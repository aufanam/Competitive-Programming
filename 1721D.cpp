...
void solve() {
        ...
        vector<int> cnta(35, 0);
        vector<int> cntb(35, 0);
        vector<vector<int>> bma(n, vector<int>(35, 0));
        vector<vector<int>> bmb(n, vector<int>(35, 0));
        for (int i = 0; i < n; i++) {
                int x = a[i];
                for (int j = 0; j < 32; j++) {
                        if (x&1) cnta[j]++, bma[i][j] = 1;
                        x >>= 1;
                }
                int y = b[i];
                for (int j = 0; j < 32; j++) {
                        if (y&1) cntb[j]++, bmb[i][j] = 1;
                        y >>= 1;
                }
        }
        vector<int> zz;
        ...
        vector<int> fin;
        vector<int> val;
        fin.push_back(zz.front());
        val.push_back((1 << zz.front()));
        int kk = (1 << zz.front());
        for (int i = 1; i < zz.size(); i++) {
                ...
                for (int j = 0; j < n; j++) {
                        ...
                        if (bmb[j][zz[i]]) res += (aa);
                        if (mapan[res^bb]) mapan[res^bb]--;
                        else bull = false;
                }
                if (bull) {
                        kk |= (aa);
                        fin.push_back(zz[i]);
                        val.push_back((1 << zz[i]));
                }
        }
        cout << kk << '\n';
        return;
