...
int main()
{
    ...
    purple = green = blue = orange = red = yellow = true;
    for (int i = 0; i < n; i++)
    {
        string s;
        cin >> s;
        if (s == "red") red = false;
        if (s == "green") green = false;
        if (s == "purple") purple = false;
        if (s == "blue") blue = false;
        if (s == "yellow") yellow = false;
        if (s == "orange") orange = false;
    }
    cout << 6 - n << '\n';
    if (red) cout << "Reality\n";
    if (blue) cout << "Space\n";
