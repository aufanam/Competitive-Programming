...
void solve() {
        ...
        for (int i = 0; i < n; i++) {
                ...
                int zz = fina.size();
                if (zz == 0) {
                        fina.push_back({a[i], x});
                } else if (a[i] == fina[zz-1].fi && zz > 0) {
                        fina.back().se += x;
                } else {
                        fina.push_back({a[i], x});
                }
        ...
        for (int i = 0; i < k; i++) {
                ...
                int zz = finb.size();
                if (zz == 0) {
                        finb.push_back({b[i], x});
                } else if (b[i] == finb[zz-1].fi) {
                        finb.back().se += x;
                } else {
                        finb.push_back({b[i], x});
                }
        }
        if (fina.size() != finb.size()) {
                cout << "No\n";
                return;
        }
        for (int i = 0; i < fina.size(); i++) {
                if (fina[i].fi != finb[i].fi) {
                        cout << "No\n";
                        return;
                }
                if (fina[i].se != finb[i].se) {
                        cout << "No\n";
                        return;
                }
        }
