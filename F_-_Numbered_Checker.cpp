...
void solve() {
        ...
        while (q--) {
                ...
                if ((a + c)%2 == 0 && (a + d)%2 == 0) {
                        ...
                        // debug(tl)
                        // debug(tr)
                        int zx = ((tl + tr)*((tr - tl)/2 + 1))/2;
                        // debug(zx);
                        int fd = zx + ((tr - tl)/2 + 1)*(m + 1) - (tr + 1 + m);
                        // debug(fd);
                        int cs = 2*m;
                        zx %= mod;
                        fd %= mod;
                        ...
                        int xx = (tr - tl)/2;
                        int bnyk = (b - a)/2;
                        ans = (ans + ((bnyk + 1)*zx))%mod;
                        // debug(ans)
                        ans = (ans + (((((((bnyk)*(bnyk + 1))/2)%mod)*cs)%mod)*wht)%mod)%mod;
                        // debug(ans)
                        bnyk = (b - (a + 1))/2;
                        if (b < a + 1) {
                                cout << ans << '\n';
                        ...
                        ans = (ans + ((bnyk + 1)*fd))%mod;
                        // debug(ans)
                        ans = (ans + (((((((bnyk)*(bnyk + 1))/2)%mod)*cs)%mod)*xx)%mod)%mod;
                        // debug(ans)
                        cout << ans << '\n';
                } else if ((a + c)%2 == 0 && (a + d)%2 == 1) {
                        tl %= mod;
                        tr = tl + d - c - 1;
                        // debug(tl)
                        // debug(tr)
                        ...
                        // debug(ans)
                        cout << ans << '\n';
                } else if ((a + c)%2 == 1 && (a + d)%2 == 0) {
                        tl %= mod;
                        tr = tl + d - c;
                        // debug(tl)
                        // debug(tr)
                        int zx = ((tl + tr + 1)*((tr - (tl + 1))/2 + 1))/2;
                        // debug(zx);
                        int fd = zx + (((tr - (tl + 1))/2 + 1))*(m - 1);
                        // debug(fd);
                        int cs = 2*m;
                        zx %= mod;
                        fd %= mod;
                        ...
                        // debug(tl)
                        // debug(tr)
                        int zx = ((tl + tr)*(((tr - 1) - (tl + 1))/2 + 1))/2;
                        // debug(zx);
                        int fd = zx + (((tr - 1) - (tl + 1))/2 + 1)*(m + 1) + (tl + m);
                        // debug(fd);
                        int cs = 2*m;
                        zx %= mod;
                        fd %= mod;
                        cs %= mod;
                        int wht = ((tr - 1) - (tl + 1))/2 + 1;
                        int xx = ((tr - 1) - (tl + 1))/2 + 2;
                        int bnyk = (b - a)/2;
                        ans = (ans + ((bnyk + 1)*zx))%mod;
                        // debug(ans)
                        ans = (ans + (((((((bnyk)*(bnyk + 1))/2)%mod)*cs)%mod)*wht)%mod)%mod;
