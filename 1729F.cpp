...
void solve() {
        ...
        pref[0] = 0;
        for (int i = 1; i <= n; i++) {
                pref[i] = (s[i] - '0' + pref[i-1])%9;
        }
        int w, q;
        cin >> w >> q;
        vector<pair<int, int>> zz(11, {-1, -1});
        for (int i = 1; i <= n - w + 1; i++) {
                int zx = (pref[i + w - 1] - pref[i - 1] + 9)%9;
                if (zz[zx].fi == -1) {
                        zz[zx].fi = i;
                } else if (zz[zx].se == -1) {
                        zz[zx].se = i;
        ...
        while (q--) {
                int l, r, k;
                cin >> l >> r >> k;
                int fd = (pref[r] - pref[l-1] + 9)%9;
                pair<int, int> ans = {INFF, INFF};
                for (int i = 0; i < 9; i++) {
                        for (int j = 0; j < 9; j++) {
                                if (i != j) {
                                        if ((i*fd + j)%9 == k) {
                                                if (zz[i].fi < ans.fi && zz[i].fi != -1 && zz[j].fi != -1) {
                                                        ans = {zz[i].fi, zz[j].fi};
                                                } else if (zz[i].fi == ans.fi && zz[j].fi < ans.se && zz[i].fi != -1 && zz[j].fi != -1) {
                                                        ans = {zz[i].fi, zz[j].fi};
                                                }
                                        }
                                } else {
                                        if ((i*fd + j)%9 == k) {
                                                if (zz[i].fi < ans.fi && zz[i].fi != -1 && zz[j].se != -1) {
                                                        ans = {zz[i].fi, zz[j].se};
                                                } else if (zz[i].fi == ans.fi && zz[j].se < ans.se && zz[i].fi != -1 && zz[j].se != -1) {
                                                        ans = {zz[i].fi, zz[j].se};
                                                }
                                        }
                                }
