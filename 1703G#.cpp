...
void solve() {
        ...
        vector<vector<int>> pre(n+5, vector<int>(36));
        for (int i = 1; i <= n; i++) {
                int x = v[i];
                for (int j = 0; j < 36; j++) {
                        pre[i][j] = x;
        ...
        vector<vector<int>> dp(n+5, vector<int>(36, -INFF));
        dp[0][0] = 0;
        dp[1][0] = v[1] - k;
        dp[1][1] = v[1]/2;
        for (int i = 1; i <= n; i++) {
                for (int j = 0; j < 36; j++) {
                        ...
                        if (i < n) dp[i+1][j] = max(dp[i+1][j], dp[i][j] + pre[i+1][j] - k);
                        if (j < 35) dp[i+1][j+1] = max(dp[i+1][j+1], dp[i][j] + pre[i+1][j+1]);
