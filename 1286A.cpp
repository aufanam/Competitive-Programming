        ...
        for (int i = 2; i <= n; i++) {
                for (int j = 0; j <= zx; j++) {
                        for (int k = 0; k <= fd; k++) {
                                if (zz[i] != 0) {
                                        if (zz[i]%2) {
                                                dp[i][j][k][1] = min({dp[i][j][k][1], dp[i-1][j][k][0] + 1, dp[i-1][j][k][1]});
                                        } else {
                                                dp[i][j][k][0] = min({dp[i][j][k][0], dp[i-1][j][k][0], dp[i-1][j][k][1] + 1});
                                        }
                                } else {
                                        if (k < fd) dp[i][j][k+1][0] = min({dp[i][j][k+1][0], dp[i-1][j][k][0], dp[i-1][j][k][1] + 1});
                                        if (j < zx) dp[i][j+1][k][1] = min({dp[i][j+1][k][1], dp[i-1][j][k][0] + 1, dp[i-1][j][k][1]});
                                }       
                        }
                }
        }
        cout << min(dp[n][zx][fd][0], dp[n][zx][fd][1]) << '\n';
        return;
}
