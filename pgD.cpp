...
vector<pair<int, int>> dist(555, {INFF, INFF}); // kiri gapake tol, kanan pake tol
void dijkstra() {
        dist[st].fi = 0;
        ...
        while (!pq.empty()) {
                ...
                for (auto z : v[p.fi.se]) {
                        if (z.se == 1) {
                                if (p.se == 0) {
                                        if (1 + p.fi.fi < dist[z.fi].fi) {
                                                dist[z.fi].fi = p.fi.fi + 1;
                                                pq.push({{dist[z.fi].fi, z.fi}, 0});
                                        }
                                } else {
                                        if (1 + p.fi.fi < dist[z.fi].se) {
                                                dist[z.fi].se = p.fi.fi + 1;
...
void solve() {
        ...
        dijkstra();
        cout << min(dist[en].fi, dist[en].se) << '\n';
        return;
}
