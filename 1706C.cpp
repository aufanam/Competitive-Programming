...
void solve() {
        ...
        int mx = ceildiv((n - 2), 2);
        if (n%2 == 0) {
                vector<vector<int>> dp(n+5, vector<int>(2, INF)); // n - udah pake shift / belum
                dp[2][0] = max(0ll, max(v[1], v[3]) - v[2] + 1);
                dp[2][1] = 0;
                dp[1][1] = 0;
                for (int i = 3; i < n; i++) {
                        if (i%2) dp[i][0] = dp[i-1][0];
                        else dp[i][0] = dp[i-1][0] + max(0ll, max(v[i-1], v[i+1]) - v[i] + 1);
                        if (i%2) dp[i][1] = min(dp[i-1][0], dp[i-2][1] + max(0ll, max(v[i-1], v[i+1]) - v[i] + 1));
                }
                cout << min(dp[n-2][0], dp[n-1][1]) << '\n';
        } else {
                int res = 0;
                for (int i = 2; i < n; i+=2) {
                        res += max(0ll, max(v[i-1], v[i+1]) - v[i] + 1);
                }
                cout << res << '\n';
        }
        return;
