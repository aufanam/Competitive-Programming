...
int qg(int l, int r, int idx, int st, int en) {
        ...
        int cs = br + 1;
        int m = st + (en - st)/2;
        return gcd(qg(l, r, br, st, m), qg(l, r, cs, m + 1, en));
}
map<int, vector<int>> mapan;
int fd(int l, int r, int x) {
        vector<int> &z = mapan[x];
        return upper_bound(all(z), r) - lower_bound(all(z), l);
}
void solve() {
        cin >> n;
        for (int i = 0; i < n; i++) {
        ...
        for (auto &x : mapan) sort(all(x.se));
        build(0, 0, n - 1);
        int q;
        cin >> q;
        while (q--) {
                ...
                l--;
                r--;
                int zx = qg(l, r, 0, 0, n - 1);
                cout << r - l + 1 - fd(l, r, zx) << '\n';
        }
        return;
}
