...
void solve() {
        ...
        for (int i = 1; i <= n; i++) {
                cin >> v[i];
                ckmax(mx, v[i]);
                males.push_back({v[i], i});
                mapan[v[i]].push_back(i);
        }
        sort(all(males));
        if (mapan[mx].size() >= 3) {
                cout << "YES\n";
                cout << mapan[mx][1] - 1 << ' ' << 1 << ' ' << n - mapan[mx][1] << '\n';
                return;
        }
        vector<int> pos;
        for (auto x : mapan) {
        ...
        for (int i = 0; i < pos.size(); i++) {
                ...
                Next:;
                if (bull) {
                        if (mapan[pos[i]][0] < idx1) {
                                if (mapan[pos[i]][mapan[pos[i]].size() - 1] >= idx2) {
                                        int l = 0;
                                        int r = mapan[pos[i]].size();
                                        bool check = false;
                                        while (true) {
                                                int m = (l+r)/2;
                                                if (mapan[pos[i]][m] >= idx2) {
                                                        r = m;
                                                } else if (mapan[pos[i]][m] < idx1) {
                                                        l = m + 1;
                                                } else {
                                                        check = true;
                                                        break;
                                        ...
                                        if (check) {
                                                cout << "YES\n";
                                                cout << idx1 - 1 << ' ' << idx2 - idx1 << ' ' << n - idx2 + 1 << '\n';
                                                return;
                                        }
                                }
                        }
