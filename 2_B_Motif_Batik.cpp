...
void solve() {
        ...
        sort(all(zz));
        for (int i = 1; i <= n; i++) {
                pref[i] = pref[i-1] + zz[i-1];
        }
        for (auto &x : v) sort(all(x));
        int ans = 0;
        for (int i = 1; i <= m; i++) {
                if (v[i].empty()) continue;
                vector<int> prefm(v[i].size() + 5, 0);
                for (int j = 1; j <= v[i].size(); j++) {
                        prefm[j] = prefm[j-1] + v[i][j-1];
                }
                for (auto z : v[i]) {
                        int res = z*((upper_bound(all(zz), z) - zz.begin()) - (upper_bound(all(v[i]), z) - v[i].begin())) - (pref[upper_bound(all(zz), z) - zz.begin()] - prefm[upper_bound(all(v[i]), z) - v[i].begin()]);
                        // debug(upper_bound(all(zz), z) - zz.begin());
                        // debug(upper_bound(all(v[i]), z) - v[i].begin());
                        // debug(pref[upper_bound(all(zz), z) - zz.begin()]);
                        // debug(prefm[upper_bound(all(v[i]), z) - v[i].begin()]);
                        ans += res;
                }
        }
        cout << ans << '\n';
