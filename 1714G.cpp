...
vector<int> zz;
vector<int> ans(N);
void dfs1(int x, int p, int cnt) {
        costa[x] = cnt;
        for (auto z : v[x]) {
                if (z.fi != p) {
                        dfs1(z.fi, x, cnt + z.se.fi);
                }
        }
}
void dfs2(int x, int p, int cnt) {
        // cout << "st\n";
        // for (auto z : zz) {
        //         cout << z << ' ';
        // }
        // cout << '\n';
        ans[x] = upper_bound(all(zz), costa[x]) - zz.begin();
        for (auto z : v[x]) {
                if (z.fi != p) {
                        // debug(z.fi)
                        zz.push_back(cnt + z.se.se);
                        dfs2(z.fi, x, cnt + z.se.se);
                        zz.pop_back();
                }
        }
}
