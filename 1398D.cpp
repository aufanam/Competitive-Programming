...
void solve() {
        ...
        sort(all(cc));
        int ans = 0;
        vector<vector<vector<int>>> dp(a+5, vector<vector<int>>(b+5, vector<int>(c+5, 0)));
        for (int i = 0; i < a + 1; i++) {
                for (int j = 0; j < b + 1; j++) {
                        for (int k = 0; k < c + 1; k++) {
                                if (i < a && j < b) {
                                        dp[i+1][j+1][k] = max(dp[i+1][j+1][k], dp[i][j][k] + aa[i]*bb[j]);
                                }
                                if (i < a && k < c) {
                                        dp[i+1][j][k+1] = max(dp[i+1][j][k+1], dp[i][j][k] + aa[i]*cc[k]);
                                }
                                if (j < b && k < c) {
                                        dp[i][j+1][k+1] = max(dp[i][j+1][k+1], dp[i][j][k] + bb[j]*cc[k]);
                                }
                                ckmax(ans, dp[i][j][k]);
                        }
                }
