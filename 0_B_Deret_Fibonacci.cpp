...
char letter(ll n)                                                       {string alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";return alphabet[n];}
string balik(string s)                                                  {reverse(s.begin(),s.end());return s;}
string base10_2(ll x)                                                   {string bruh="";if(x==0)return "0";while(x){ll nih=x%(-2);x/=-2;if(nih<0)nih+=2,x++;bruh+=to_string(nih);}reverse(all(bruh));return bruh;}
...
void rec() {
        ...
        for (int i = 0; i < b.size(); i++) {
                int x = a[i] + b[i] - 96 + sisa;
                sisa = 0;
                if (x >= 10) sisa++, x -= 10;
                c.push_back(letter(x + 52));
        }
        if (a.size() > b.size()) {
                c.push_back(letter(a[a.length()-1] + 4 + sisa));
        } else if (sisa > 0) {
                c.push_back('1');
        }
        reverse(all(c));
...
void solve() {
        ...
        v.push_back("0");
        v.push_back("1");
        while (v[cnt-1] != s and v[cnt-1].length() <= s.length()) rec();
        // cout << v[0] << ' ' << v[1] << ' ' << v[2] << '\n';
        // cout << v[0][0] + v[1][0] << '\n';
        // cout << letter(53) << '\n';
        if (v[cnt-1].length() > s.length()) cout << "0\n";
        else cout << cnt << '\n';
        return;
}
