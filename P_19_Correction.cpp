...
void solve() {
        ...
        string x;
        cin >> x;
        vector<vector<int>> pre(n, vector<int>(3005, -1));
        for (int i = 0; i < n; i++) {
                int res = 0;
                for (int j = 0; j < min((ll)v[i].length(), (ll)x.length()); j++) {
                        if (x[j] > v[i][j]) {
                                res += min(x[j] - v[i][j], v[i][j] + 26 - x[j]);
                        } else {
                                res += min(v[i][j] - x[j], x[j] + 26 - v[i][j]);
                        }
                        pre[i][j] = res;
                }
        }
