...
void solve() {
        ...
        for (int i = 1; i*i <= cnt; i++) {
                if (cnt%i == 0) {
                        zz.push_back({i, cnt/i});
                        zz.push_back({cnt/i, i});
                }
        }
        vector<vector<int>> prefr(n+5, vector<int>(m+5, 0));
        for (int i = 1; i <= n; i++) {
                prefr[i][1] = v[i][1];
        }
        for (int i = 1; i <= n; i++) {
                for (int j = 2; j <= m; j++) {
                        prefr[i][j] = prefr[i][j-1] + v[i][j];
                }
        }
        vector<vector<int>> pref(n+5, vector<int>(m+5));
        for (int i = 1; i <= m; i++) {
                pref[1][i] = prefr[1][i];
        }
        for (int i = 2; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                        pref[i][j] = pref[i-1][j] + prefr[i][j];
                }
        }
        int ans = INF;
        for (auto z : zz) {
                if (z.fi > n || z.se > m) continue;
                for (int i = z.fi; i <= n; i++) {
                        for (int j = z.se; j <= m; j++) {
                                int res = 0;
                                res += pref[i][j];
                                res -= pref[i-z.fi][j];
                                res -= pref[i][j-z.se];
                                res += pref[i-z.fi][j-z.se];
                                int zx = cnt - res;
                                ckmin(ans, zx);
                        }
                }
