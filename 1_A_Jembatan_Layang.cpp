...
void solve() {
        ...
        int n, k;
        cin >> n >> k;
        vector<pair<int, int>> rng1(n+5, {0, 0});
        vector<pair<int, int>> rng2(n+5, {0, 0});
        vector<int> bridge(k+5);
        vector<pair<bool, bool>> ok(n+5, {false, false});
        for (int i = 1; i <= k; i++) {
                cin >> bridge[i];
        }
        for (int i = 1; i <= n; i++) {
        ...
        for (int i = 1; i <= k; i++) {
                if (!ok[i].fi && !ok[i].se) {
                        if (rng1[i].fi == rng2[i].se && rng1[i].se == rng2[i].fi) {
                                ans = ans * fac[rng1[i].fi] % MOD * fac[rng2[i].fi] % MOD;
                        } else {
                                ans *= 0;
                        }
                } else if (ok[i].fi && !ok[i].se) {
                        if ((rng1[i].fi + 1 == rng2[i].se && rng1[i].se == rng2[i].fi) || (rng1[i].fi == rng2[i].se && rng1[i].se + 1 == rng2[i].fi)) {
                                ans = ans * fac[rng2[i].fi] % MOD * fac[rng2[i].se] % MOD;
                        } else {
                                ans *= 0;
                        }
                } else if (!ok[i].fi && ok[i].se) {
                        if ((rng1[i].fi == rng2[i].se + 1 && rng1[i].se == rng2[i].fi) || (rng1[i].fi == rng2[i].se && rng1[i].se == rng2[i].fi + 1)) {
                                ans = ans * fac[rng1[i].fi] % MOD * fac[rng1[i].se] % MOD;
                        } else {
                                ans *= 0;
                        }
                } else {
                        if (rng1[i].fi == rng2[i].se && rng1[i].se == rng2[i].fi) {
                                ans = ans * fac[rng1[i].fi] % MOD * fac[rng1[i].se] % MOD * (rng1[i].fi + rng1[i].se + 1) % MOD;
                        } else if (rng1[i].fi == rng2[i].se + 1 && rng2[i].se + 1 == rng2[i].fi) {
                                ans = ans * fac[rng1[i].fi] % MOD * fac[rng2[i].fi] % MOD;
                        } else if (rng1[i].fi + 1 == rng2[i].se && rng1[i].se == rng2[i].fi + 1) {
                                ans = ans * fac[rng1[i].se] % MOD * fac[rng2[i].se] % MOD;
                        } else {
                                ans *= 0;
                        }
                }
