...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            mapan.insert({mp(v[i].first, v[i].second), 0});
        }
        for (int i = 0; i < n; i++)
        {
            mapan[mp(v[i].first, v[i].second)]++;
        }
        ll ans = 0;
        for (int i = 0; i < n; i++)
        {
            ...
            {
                ll y = mapan[mp(v[i].first, v[i].second)] - 1;
                ans += y;
                continue;
            }
            if (mapan[mp(v[i].first, v[i].second)] > 0 && mapan[mp(v[i].second, v[i].first)] > 0)
            {
                ll z = mapan[mp(v[i].second, v[i].first)];
                ans += z;
            }
        }
        ans /= 2;
