...
void solve() {
        ...
        vector<int> ans;
        int zx = gcd(zz[0], zz[1]);
        int fd = gcd(zz[2], zz[3]);
        int cs = gcd(zx, fd);
        set<int> buset = {4, 8, 15, 16, 23, 42};
        if (buset.count(cs)) ans.push_back(cs);
        else ans.push_back(cs/2), cs /= 2;
        for (int i = 2; i <= 5; i++) {
                ans.push_back(zz[i-2]/cs);
        }
        for (auto x : ans) {
                buset.erase(x);
        }
