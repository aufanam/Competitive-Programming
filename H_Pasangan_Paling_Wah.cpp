...
void solve() {
        int n;
        cin >> n;
        map<int, set<int>> mboys;
        map<int, set<int>> mwemen;
        vector<int> boys(n);
        vector<int> wemen(n);
        for (int i = 0; i < n; i++) {
                int x;
        ...
        int rw = n - 1;
        for (int i = 0; i < n; i++) {
                if (abs(boys[lb] - wemen[rw]) > abs(boys[rb] - wemen[lw])) {
                        cout << *mboys[boys[lb]].begin() << ' ' << *mwemen[wemen[rw]].begin() << ' ' << abs(boys[lb] - wemen[rw])*abs(boys[lb] - wemen[rw]) << '\n';
                        mboys[boys[lb]].erase(mboys[boys[lb]].begin());
                        mwemen[wemen[rw]].erase(mwemen[wemen[rw]].begin());
                        lb++;
                        rw--;
                } else if (abs(boys[lb] - wemen[rw]) < abs(boys[rb] - wemen[lw])) {
                        cout << *mboys[boys[rb]].begin() << ' ' << *mwemen[wemen[lw]].begin() << ' ' << abs(boys[rb] - wemen[lw])*abs(boys[rb] - wemen[lw]) << '\n';
                        mboys[boys[rb]].erase(mboys[boys[rb]].begin());
                        mwemen[wemen[lw]].erase(mwemen[wemen[lw]].begin());
                        rb--;
                        lw++;
                } else if (*mboys[boys[lb]].begin() < *mboys[boys[rb]].begin()) {
                        cout << *mboys[boys[lb]].begin() << ' ' << *mwemen[wemen[rw]].begin() << ' ' << abs(boys[lb] - wemen[rw])*abs(boys[lb] - wemen[rw]) << '\n';
                        mboys[boys[lb]].erase(mboys[boys[lb]].begin());
                        mwemen[wemen[rw]].erase(mwemen[wemen[rw]].begin());
                        lb++;
                        ...
                        rb--;
                        lw++;
                } else if (*mwemen[wemen[rw]].begin() < *mwemen[wemen[lw]].begin()) {
                        cout << *mboys[boys[lb]].begin() << ' ' << *mwemen[wemen[rw]].begin() << ' ' << abs(boys[lb] - wemen[rw])*abs(boys[lb] - wemen[rw]) << '\n';
                        mboys[boys[lb]].erase(mboys[boys[lb]].begin());
                        mwemen[wemen[rw]].erase(mwemen[wemen[rw]].begin());
                        lb++;
