...
int sz;
set<int> buset;
int toint(string sss) {
        int res = 0;
        reverse(all(sss));
        for (int i = 0; i < sss.length(); i++) {
                res += (sss[i] - '0')*pow(10, i);
        }
        return res;
}
void rec(int cnt, int x, string ss, bool ok) {
        if (x == 0) {
                string z = "";
                z.push_back(s[0]);
                // debug(z)
                ...
                ss.push_back(s[x]);
                rec(cnt + toint(ss), x + 1, "", false);
                if (!ok) rec(cnt - toint(ss), x + 1, "", ok);
                // debug(cnt)debug(ss)
                rec(cnt, x + 1, ss, ok);
        } else {
                buset.insert(cnt + toint(ss) + (s[x] - '0'));
                buset.insert(cnt + toint(ss) - (s[x] - '0'));
                if (!ok) buset.insert(cnt - toint(ss) + (s[x] - '0'));
                if (!ok) buset.insert(cnt - toint(ss) - (s[x] - '0'));
                ss.push_back(s[x]);
                buset.insert(cnt + toint(ss));
                if (!ok) buset.insert(cnt - toint(ss));
        }
}
void solve() {
        cin >> s;
