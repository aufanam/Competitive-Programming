...
void solve() {
        ...
        sort(allr(enol));
        sort(allr(satu));
        if (enol.size() < satu.size()) {
                int z = enol.size();
                ans += accumulate(all(enol), 0ll);
                ans += accumulate(satu.begin(), satu.begin() + z, 0ll);
        } else if (satu.size() < enol.size()) {
                int z = satu.size();
                ans += accumulate(all(satu), 0ll);
                ans += accumulate(enol.begin(), enol.begin() + z, 0ll);
        } else {
                int zx = accumulate(all(enol), 0ll) + accumulate(all(satu), 0ll);
                ans += max(zx - satu.back(), zx - enol.back());
        }
        cout << ans << '\n';
        return;
}
