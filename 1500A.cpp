...
void solve() {
        ...
        for (int i = 1; i <= n; i++) {
                ...
                if (cnt == 2) {
                        cout << "YES\n";
                        cout << idx[ok[0]][0] << ' ' << idx[ok[1]][0] << ' ' << idx[ok[0]][1] << ' ' << idx[ok[1]][1] << '\n';
                        return;
                }
        }
        sort(all(v));
        if (cnt == 1) {
                ...
                while (true) {
                        ...
                        while (v[r].fi == ok[0] || l == r) {
                                r++;
                        }
                        int zz = v[l].fi + v[r].fi;
                        if (zz == xx) {
                                cout << "YES\n";
                                cout << v[l].se << ' ' << v[r].se << ' ' << idx[ok[0]][0] << ' ' << idx[ok[0]][1] << '\n';
                                return;
                        } else if (zz < xx) {
                                r++;
                        } else {
        ...
        set<int> buset;
        for (int i = 1; i <= n; i++) {
                if (!buset.count(v[i].fi)) {
                        buset.insert(v[i].fi);
                        pos.push_back({v[i].fi, v[i].se});
                }
        }
        vector<vector<pair<int, int>>> cc(5555555);
        int yy = pos.size();
        for (int i = 0; i < yy; i++) {
                for (int j = i + 1; j < yy; j++) {
                        int kk = pos[i].fi + pos[j].fi;
                        cc[kk].push_back({pos[i].se, pos[j].se});
                        if (cc[kk].size() == 2) {
                                cout << "YES\n";
                                for (auto mm : cc[kk]) {
                                        cout << mm.fi << ' ' << mm.se << ' ';
