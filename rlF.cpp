...
map<string, bool> mapan;
vector<vector<int>> ans(150, vector<int>(150));
void rec(string x, int x1, int y1, int x2, int y2) {
        if (x1 == x2 && y1 == y2) {
                x = "1" + x;
                if (mapan[x]) ans[x1][y1] = 1;
                else ans[x1][y1] = 0;
                return;
        }
        rec(x + "0", x1, y1, (x1 + x2)/2, (y1 + y2)/2);
        rec(x + "1", x1, ceildiv(y1 + y2, 2), (x1 + x2)/2, y2);
        rec(x + "2", ceildiv(x1 + x2, 2), y1, x2, (y1 + y2)/2);
        rec(x + "3", ceildiv(x1 + x2, 2), ceildiv(y1 + y2, 2), x2, y2);
}
void find(string x, int ln) {
        if (x.length() == ln) { 
                mapan[x] = true;
        } else {
                find(x + "0", ln);
...
void solve() {
        ...
        int n, m;
        cin >> n >> m;
        int z = max(pow(2, ceil(log2(n))), pow(2, ceil(log2(m))));
        int ln = max(ceil(log2(n)), ceil(log2(m))) + 1;
        for (int i = 0; i < k; i++) {
                string x = v[i];
                find(x, ln);
        }
