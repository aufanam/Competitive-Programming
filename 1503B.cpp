...
int n;
vector<vector<int>> v;
vector<vector<int>> zx;
void find(int a) {
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                        if (v[i][j] == -1 && zx[i][j] != a) {
                                v[i][j] = zx[i][j];
                                cout << zx[i][j] << ' ' << i + 1 << ' ' << j + 1 << endl;
                                return;
                        }
                }
        }
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                        if (v[i][j] == -1 && zx[i][j] == a) {
                                bool b1, b2, b3, b4;
                                b1 = b2 = b3 = b4 = true;
                                if (i > 0) if (v[i-1][j] == 3) b1 = false;
                                if (i < n - 1) if (v[i+1][j] == 3) b2 = false;
                                if (j > 0) if (v[i][j-1] == 3) b3 = false;
                                if (j < n - 1) if (v[i][j+1] == 3) b4 = false;
                                if (b1 and b2 and b3 and b4) {
                                        v[i][j] = 3;
                                        cout << v[i][j] << ' ' << i + 1 << ' ' << j + 1 << endl;
                                        return;
                                }
                        }
                }
...
void solve() {
        ...
        zx = vector<vector<int>> (n, vector<int>(n));
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                        zx[i][j] = modx(i + j + 1, 2ll);
                }
        }
        for (int i = 0; i < n*n; i++) {
                int x;
