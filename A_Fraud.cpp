...
void solve() {
        ...
        pair<pair<int, int>, pair<int, int>> chk = {{0, 1}, {INF, 1}};
        for (int i = 0; i < n - 1; i++) {
                if (v[i].fi < v[i+1].fi && v[i].se < v[i+1].se) {
                        cout << "NO\n";
                        return;
                }
                int dif1 = abs(v[i].fi - v[i+1].fi);
                int dif2 = abs(v[i].se - v[i+1].se);
                if (v[i].fi > v[i+1].fi && v[i].se < v[i+1].se) {
                        if (chk.fi.fi*dif1 < dif2*chk.fi.se) {
                                chk.fi.fi = dif2;
                                chk.fi.se = dif1;
                        }
                } else if (v[i].fi < v[i+1].fi && v[i].se > v[i+1].se) {
                        if (chk.se.fi*dif1 > dif2*chk.se.se) {
                                chk.se.fi = dif2;
                                chk.se.se = dif1;
                        }
                }
        }
        int val1 = chk.fi.fi*chk.se.se;
        int val2 = chk.se.fi*chk.fi.se;
        cout << (val1 < val2 ? "YES\n" : "NO\n");
        return;
}
