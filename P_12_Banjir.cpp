...
void solve() {
        ...
        while (!q.empty()) {
                ...
                q.pop();
                safe[p.fi][p.se] = false;
                vis[p.fi][p.se] = true;
                for (int k = 0; k < 4; k++) {
                        if (1 <= p.fi + dx[k] && p.fi + dx[k] <= n && 1 <= p.se + dy[k] && p.se + dy[k] <= m && !v[p.fi + dx[k]][p.se + dy[k]] && !vis[p.fi + dx[k]][p.se + dy[k]]) {
                                safe[p.fi + dx[k]][p.se + dy[k]] = false;
