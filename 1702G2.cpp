...
void dfs(int cur, int prev) {
        ...
        memo[cur][0] = prev;
        for (int i = 1; i < lg; i++) {
                memo[cur][i] = memo[memo[cur][i-1]][i-1];
        }
        for (auto z : v[cur]) {
                if (z != prev) {
                        dfs(z, cur);
...
int lca(int a, int b) {
        ...
        for (int i = lg - 1; i >= 0; i--) {
                if (!isAncestor(memo[a][i], b)) {
                        a = memo[a][i];
                }
        }
        return memo[a][0];
}
const bool comp(const int &a, const int &b) {return (tin[a] < tin[b]);}
void solve() {
        // credit: Zanite - nice solution 
        // nvm idiot me
        tout[0] = INF;
        cin >> n;
        for (int i = 0; i < n - 1; i++) {
        ...
        while (q--) {
                ...
                vector<int> st;
                for (auto zz : z) {
                        while (!st.empty() && !isAncestor(st.back(), zz)) {
                                st.pop_back();
                        }
                        if (!st.empty()) {
                                deg[st.back()]++;
