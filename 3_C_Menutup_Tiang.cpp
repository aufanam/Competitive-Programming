...
void solve() {
        ...
        vector<int> rg;
        int r = 0;
        for (int i = n - 1; i >= lf[lf.size()-1]; i--) {
                if (h[i] >= r) {
                        rg.push_back(i);
                        r = h[i];
                }
        ...
        for (int i = 1; i < lf.size(); i++) {
                // debug(v[lf[i]].fi)
                // debug(h[lf[i]])
                if (!doIntersect({v[lf[pos]].fi, 0}, {v[lf[pos]].fi, h[lf[pos]]}, {xlst, hlst}, {v[lf[i]].fi, h[lf[i]]})) {
                        pos = lf[i];
                } else {
                        // cout << sqrt((v[lf[pos]].fi - xlst)*(v[lf[pos]].fi - xlst) + (h[lf[pos]] - hlst)*(h[lf[pos]] - hlst)) << '\n';
                        ans += sqrt((v[lf[pos]].fi - xlst)*(v[lf[pos]].fi - xlst) + (h[lf[pos]] - hlst)*(h[lf[pos]] - hlst));
                        xlst = v[lf[pos]].fi;
                        hlst = h[lf[pos]];
                        pos = lf[i];
                }
        ...
        for (int i = 1; i < rg.size(); i++) {
                // debug(v[rg[i]].se)
                // debug(h[rg[i]])
                if (!doIntersect({v[rg[pos]].se, 0}, {v[rg[pos]].se, h[rg[pos]]}, {xlst, hlst}, {v[rg[i]].se, h[rg[i]]})) {
                        pos = i;
                } else {
                        // debug(pos);
                        // cout << sqrt((v[rg[pos]].se - xlst)*((v[rg[pos]].se - xlst)) + (h[rg[pos]] - hlst)*(h[rg[pos]] - hlst)) << '\n';
                        ans += sqrt((v[rg[pos]].se - xlst)*((v[rg[pos]].se - xlst)) + (h[rg[pos]] - hlst)*(h[rg[pos]] - hlst));
                        xlst = v[rg[pos]].se;
                        hlst = h[rg[pos]];
                        pos = i;
                        // debug(i)
