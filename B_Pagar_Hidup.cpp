...
void solve() {
        ...
        for (int i = 0; i < n - 1; i++) {
                ...
                int cs = fd - zx;
                int br = zx + cs/3;
                int tl = zx + ceildiv(cs, 3);
                if (range(zx, br, fd)) ckmax(ans, min(2*(br - zx), fd - br)); // oalah harusnya min bkn max -_-
                if (range(zx, tl, fd)) ckmax(ans, min(2*(tl - zx), fd - tl));
        }
        cout << ans << '\n';
        return;
}
