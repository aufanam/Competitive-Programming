...
vector<vector<pair<int, int>>> v(222222);
vector<int> dist(222222, INFF);
vector<int> meh(222222, 0);
vector<pair<int, int>> flights(222222, {INFF, -INFF}); // min dan max
map<pair<int, int>, bool> vis;
void dijkstra() {
        dist[1] = 0;
        meh[1] = 1;
        priority_queue<pair<pair<int, int>, pair<int, int>>, vector<pair<pair<int, int>, pair<int, int>>>, greater<pair<pair<int, int>, pair<int, int>>>> pq;
        pq.push({{0, 1}, {0, 0}}); // price - node - flights - lst
        while (!pq.empty()) {
                auto p = pq.top();
                pq.pop();
                if (dist[p.fi.se] < p.fi.fi) continue;
                bool bull = false;
                if (p.se.fi < flights[p.fi.se].fi) {
                        flights[p.fi.se].fi = p.se.fi;
                        bull = true;
                }
                if (p.se.fi > flights[p.fi.se].se) {
                        flights[p.fi.se].se = p.se.fi;
                        bull = true;
                } 
                if (!bull) continue;
                for (auto z : v[p.fi.se]) {
                        if (p.fi.fi + z.se <= dist[z.fi]) {
                                dist[z.fi] = p.fi.fi + z.se;
                                pq.push({{dist[z.fi], z.fi}, {p.se.fi + 1, p.fi.se}});
                        }
                }
        }
}
void solve() {
        ...
        dijkstra();
        cout << dist[n] << ' ' << meh[n] << ' ' << flights[n].fi << ' ' << flights[n].se << '\n';
        return;
}
