...
int dx[] = {1, 0, -1, 0};
int dy[] = {0, 1, 0, -1};
int rec(vector<vector<int>> &v) {
        vector<vector<bool>> vis(25, vector<bool>(25, false));
        vector<vector<int>> zz(25, vector<int>(25));
        zz = v;
        int ans = 0;
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                        if (vis[i][j] || v[i][j] == -1) continue;
                        ...
                        while (!q.empty()) {
                                ...
                                zz[p.fi][p.se] = -1;
                                for (int k = 0; k < 4; k++) {
                                        if (0 <= p.fi + dx[k] && p.fi + dx[k] < n && 0 <= p.se + dy[k] && p.se + dy[k] < m && !vis[p.fi + dx[k]][p.se + dy[k]] && v[p.fi + dx[k]][p.se + dy[k]] == v[p.fi][p.se]) {
                                                cnt++;
                                                q.push({p.fi + dx[k], p.se + dy[k]});
                                                vis[p.fi + dx[k]][p.se + dy[k]] = true;
                                        }
                                }
                        }
                        if (cnt == 1) {
                                zz[p.fi][p.se] = v[p.fi][p.se];
                                continue;
                        }
                        for (int ki = 0; ki < m; ki++) {
                                for (int kj = n - 2; kj >= 0; kj--) {
                                        if (zz[kj][ki] != -1) {
                                                int x = kj;
                                                int y = ki;
                                                while (zz[x+1][y] == -1 && x < n - 1) {
                                                        swap(zz[x][y], zz[x+1][y]);
                                                        x++;
                                                }
                                        }
                                }
                        }
                        ckmax(ans, cnt*(cnt - 1) + rec(zz));
...
void solve() {
        // https://tlx.toki.id/courses/competitive/chapters/09/problems/A - Maze
        cin >> n >> m;
