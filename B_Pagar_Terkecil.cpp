...
void solve() {
        ...
        vector<pair<pair<int, int>, pair<int, int>>> pref(n+5);
        vector<pair<pair<int, int>, pair<int, int>>> suf(n+5);
        for (int i = 0; i < n; i++) {
                cin >> v[i].fi >> v[i].se;
        }
        pref[0] = {{v[0].fi, v[0].fi}, {v[0].se, v[0].se}};
        for (int i = 1; i < n; i++) {
                pref[i].fi.fi = max(pref[i-1].fi.fi, v[i].fi);
                pref[i].fi.se = min(pref[i-1].fi.se, v[i].fi);
                pref[i].se.fi = max(pref[i-1].se.fi, v[i].se);
                pref[i].se.se = min(pref[i-1].se.se, v[i].se);
        }
        suf[n-1] = {{v[n-1].fi, v[n-1].fi}, {v[n-1].se, v[n-1].se}};
        for (int i = n - 2; i >= 0; i--) {
                suf[i].fi.fi = max(suf[i+1].fi.fi, v[i].fi);
                suf[i].fi.se = min(suf[i+1].fi.se, v[i].fi);
                suf[i].se.fi = max(suf[i+1].se.fi, v[i].se);
                suf[i].se.se = min(suf[i+1].se.se, v[i].se);
        }
        int ans = INFF;
        ckmin(ans, (pref[n-2].fi.fi - pref[n-2].fi.se)*(pref[n-2].se.fi - pref[n-2].se.se));
        ckmin(ans, (suf[1].fi.fi - suf[1].fi.se)*(suf[1].se.fi - suf[1].se.se));
        for (int i = 1; i < n - 1; i++) {
                int res = (max(pref[i-1].fi.fi, suf[i+1].fi.fi) - min(pref[i-1].fi.se, suf[i+1].fi.se))*(max(pref[i-1].se.fi, suf[i+1].se.fi) - min(pref[i-1].se.se, suf[i+1].se.se));
                ckmin(ans, res);
        }
        cout << ans << '\n';
        return;
