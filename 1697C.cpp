...
void solve() {
        ...
        string a, b;
        cin >> n >> a >> b;
        int aa = count(all(a), 'a');
        int bb = count(all(a), 'b');
        int cc = count(all(a), 'c');
        int aaa = count(all(b), 'a');
        int bbb = count(all(b), 'b');
        int ccc = count(all(b), 'c'); 
        ...
        for (int i = 0; i < idx.size() - 1; i++) {
                ...
                if (fitype == 1) {
                        for (int j = idx[i]; j < idx[i+1]; j++) {
                                if (a[j] == 'b') cnta.push_back(j);
                                if (b[j] == 'b') cntb.push_back(j);
                        }
                        // debug
                        // print(cnta);
                        // print(cntb);
