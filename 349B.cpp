...
void solve() {
        ...
        for (int i = 1; i <= 9; i++) {
                if (pos[i].size() == mx) {
                        pass.push_back(pos[i]);
                }
        }
        int mxx = 0;
        for (int i = 0; i < pass.size(); i++) {
                int num = pass[i][0];
                int rem = m - pass[i].size()*(v[num]) + v[num];
                int cnt = 0;
                for (int j = 9; j >= 1; j--) {
                        while (rem >= v[j] && j > num && cnt < pass[i].size()) {
                                pass[i][cnt] = j;
                                cnt++;
                                rem -= (v[j] - v[num]);
                        }
        ...
        for (int i = 0; i < pass.size(); i++) {
                if (pass[i].size() == mxx) {
                        final.push_back(pass[i]);
                }
        }
        int mxxx = 0;
        for (int i = 0; i < final.size(); i++) {
                int zz = final[i][0]*10 + final[i][1];
                ckmax(mxxx, zz);
        }
        for (int i = 0; i < final.size(); i++) {
                int zz = final[i][0]*10 + final[i][1];
