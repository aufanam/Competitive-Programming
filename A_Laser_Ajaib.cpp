...
vector<vector<int>> v(111, vector<int>(111, 0));
bool kena;
int n, m;
void decul(int x, int y) {
        if (y == m) {
                kena = true;
                return;
        }
        if (range(1, x - 1, n) && v[x-1][y+1] == 1) decul(x-1, y+1);
        if (range(1, x, n) && v[x][y+1] == 1) decul(x, y+1);
        if (range(1, x + 1, n) && v[x+1][y+1] == 1) decul(x+1, y+1);
}
void solve() {
        cin >> n >> m;
