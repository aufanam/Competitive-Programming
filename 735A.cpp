...
void solve() {
        int lengthOfString, numberOfJumpsPerMove;
        cin >> lengthOfString >> numberOfJumpsPerMove;
        string theString;
        cin >> theString;
        int insect = theString.find('T');
        int grasshopper = theString.find('G');
        int theLeftMost = min(insect, grasshopper);
        int theRightMost = max(insect, grasshopper);
        if ((theRightMost-theLeftMost)%numberOfJumpsPerMove) {
                cout << "NO\n";
                return;
        }
        for (int i = theLeftMost; i <= theRightMost; i+=numberOfJumpsPerMove) {
                if (theString[i] == '#') {
                        cout << "NO\n";
                        return;
                }
