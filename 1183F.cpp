...
void solve() {
        ...
        if (mx%2 == 0 && mx%3 == 0 && mx%5 == 0) {
                if (buset.count(mx/2) && buset.count(mx/3) && buset.count(mx/5)) {
                        ckmax(ans, mx/2 + mx/3 + mx/5);
                }
        }
        vector<int> res;
        while (!buset.empty() && res.size() < 3) {
                int zx = *buset.rbegin();
                buset.erase(prev(buset.end()));
                bool bull = true;
                for (auto it : res) bull &= (it%zx != 0);
                if (bull) res.push_back(zx);
        }
        ckmax(ans, accumulate(all(res), 0ll));
        cout << ans << '\n';
        return;
}
