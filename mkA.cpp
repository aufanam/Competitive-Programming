...
const int oeisA003226[] = {1, 5, 6, 25, 76, 376, 625, 9376, 90625, 109376, 890625};
void solve() {
        string n;
        cin >> n;
        for (int i = 0; i < 11; i++) {
                string s = to_string(oeisA003226[i]);
                int res = stoll(n + s);
                int len = s.length();
                int z = stoll(s);
                for (int j = 10; j*j <= res; j++) {
                        if (res%j == 0) {
                                int zx = j;
                                int fd = res/j;
                                int zz = binpow(10, len, INFF*2);
                                if (zx%zz == z && fd%zz == z) {
                                        if (zx < 10) continue;
                                        cout << "YA\n";
                                        cout << zx/zz << ' ' << fd/zz << ' ' << oeisA003226[i] << '\n';
                                        return;
                                }
                        }
                }
