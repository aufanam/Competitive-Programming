...
void solve() {
    ...
    vector<vector<int>> sus(n, vector<int>(m));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cin >> v[i][j];
            sus[i][j] = v[i][j];
    ...
    for (int i = 0; i < m; i++) {
        bool bull = false;
        for (int j = 0; j < n; j++) {
            if (v[j][i] != sus[j][i]) bull = true;
        }
        if (bull) tuker.push_back(i);
    }
    if (tuker.empty()) cout << "1 1\n";
    else if (tuker.size() == 2) {
        for (int i = 0; i < n; i++) {
            swap(v[i][tuker[0]], v[i][tuker[1]]);
        }
        bool bull = true;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (v[i][j] != sus[i][j]) bull = false;
            }
        }
        if (bull) cout << tuker[0] + 1 << ' ' << tuker[1] + 1 << '\n';
        else cout << "-1\n";
    }
    else cout << "-1\n";
    return;
