...
int xs, ys;
int xe, ye;
vector<vector<char>> v(1111, vector<char>(1111));
vector<vector<int>> dist(1111, vector<int>(1111, INFF));
void dijkstra() {
        ...
        while (!pq.empty()) {
                ...
                pq.pop();
                if (range(1, p.se.fi + 1, n) && v[p.se.fi + 1][p.se.se] == '#' && p.fi < dist[p.se.fi + 1][p.se.se]) {
                        pq.push({p.fi, {p.se.fi + 1, p.se.se}});
                        dist[p.se.fi + 1][p.se.se] = p.fi;
                } else {
                        int x = p.se.fi + 1;
                        int y = p.se.se;
                        while (v[x][y] != '#' && range(1, x, n) && range(1, y, m)) {
                                x++;
                        }
                        if (range(1, x, n) && range(1, y, m) && p.fi + 1 < dist[x][y]) {
                                pq.push({p.fi + 1, {x, y}});
                                dist[x][y] = p.fi + 1;
                        }
                }
                ...
                if (range(1, p.se.se + 1, m) && v[p.se.fi][p.se.se + 1] == '#' && p.fi < dist[p.se.fi][p.se.se + 1]) {
                        pq.push({p.fi, {p.se.fi, p.se.se + 1}});
                        dist[p.se.fi][p.se.se + 1] = p.fi;
                } else {
                        int x = p.se.fi;
                        int y = p.se.se + 1;
                        while (v[x][y] != '#' && range(1, x, n) && range(1, y, m)) {
...
void solve() {
        ...
        //         cout << '\n';
        // }
        if (dist[xe][ye] != INFF) cout << dist[xe][ye] << '\n';
        else cout << "-1\n";
        return;
}
