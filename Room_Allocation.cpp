...
void solve() {
        int n;
        cin >> n;
        vector<pair<pair<int, int>, int>> zz(n);
        for (int i = 0; i < n; i++) {
                int a, b;
                cin >> a >> b;
                zz[i] = {{a, b}, i};
        ...
        for (int i = 0; i < n; i++) {
                ...
                for (auto it = setdah.begin(); it != setdah.end(); it++) {
                        if (it->fi < st) {
                                buset.insert(it->se);
                                tbe.push_back({it->fi, it->se});
                        } else {
                                break;
                        }
                }
                for (auto z : tbe) {
                        setdah.erase({z.fi, z.se});
                }
                ans[zz[i].se] = *buset.begin();
                setdah.insert({en, *buset.begin()});
                buset.erase(buset.begin());
        }
