...
#define xx  ((j-1)%(grup[i].size()) + grup[i].size())%grup[i].size()
using namespace std;
...
void solve() {
        ...
        for (int i = 0; i < grup.size(); i++) {
                ...
                for (int j = 0; j < grup[i].size(); j++) {
                        let[grup[i][j]] = w[grup[i][xx]];
                }
                int move = 0;
                bool nice = true;
                do {
                        move++;
                        let[grup[i][0]] = w[v[grup[i][0]]];
                        for (int j = 1; j < grup[i].size(); j++) {
                                let[grup[i][j]] = w[v[grup[i][j]]];
                                w[grup[i][j]] = let[grup[i][j]];
                        }
                        w[grup[i][0]] = let[grup[i][0]];
                        bool bull = true;
                        for (int j = 0; j < grup[i].size(); j++) {
                                if (let[grup[i][j]] != s[grup[i][j]]) {
                                        bull = false;
                                }
                        }
                        if (bull) {
