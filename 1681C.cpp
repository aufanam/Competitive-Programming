...
void solve()
{
    ...
    for (auto &x : a) cin >> x;
    for (auto &x : b) cin >> x;
    vector<int> mn(101, 123);
    vector<int> mx(101, -123);
    for (int i = 0; i < n; i++) {
        int z;
        z = a[i];
        ckmin(mn[z], b[i]);
    ...
    for (int i = 0; i < n; i++) {
        bruh.push_back(mp(a[i], b[i]));
    }
    sort(all(bruh));
    int cnt = 0;
    vector<pair<int, int>> ans;
    for (int i = 0; i < n; i++) {
        if (a[i] == bruh[i].first && b[i] == bruh[i].second) continue;
        for (int j = i + 1; j < n; j++) {
            if (a[j] == bruh[i].first && b[j] == bruh[i].second) {
                cnt++;
