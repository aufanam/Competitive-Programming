...
void solve() {
        ...
        int dp[100005][2];
        dp[0][0] = 1;
        dp[0][1] = (v[0].fi + v[0].se < v[1].fi);
        for (int i = 1; i < n - 1; i++) {
                dp[i][0] = max(dp[i-1][0] + (v[i].fi - v[i].se > v[i-1].fi), dp[i-1][1] + (v[i].fi - v[i].se > v[i-1].fi + v[i-1].se));
                dp[i][1] = max(dp[i-1][0] + (v[i].fi + v[i].se < v[i+1].fi), dp[i-1][1] + (v[i].fi + v[i].se < v[i+1].fi));
        }
        int ans = max(dp[n-2][0], dp[n-2][1]) + 1;
        cout << ans << '\n';
        return;
}
