...
void solve() {
        // dah ah
        // baca soal lebih teliti lg ya hdh
        int n, m;
        cin >> n >> m;
        vector<vector<char>> v(555, vector<char>(555));
        int kx, ky, bx, by;
        for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                        cin >> v[i][j];
        ...
        int dx[] = {1, 0, -1, 0};
        int dy[] = {0, 1, 0, -1};
        vector<vector<bool>> visk(555, vector<bool>(555));
        vector<vector<bool>> visb(555, vector<bool>(555));
        while (!q.empty()) {
                pair<int, int> p = q.front();
                q.pop();
                visk[p.fi][p.se] = true;
                for (int k = 0; k < 4; k++) {
                        if (0 <= p.fi + dx[k] && p.fi + dx[k] < m && 0 <= p.se + dy[k] && p.se + dy[k] < n && v[p.fi + dx[k]][p.se + dy[k]] == '.' && !visk[p.fi + dx[k]][p.se + dy[k]]) {
                                resk++;
                                visk[p.fi + dx[k]][p.se + dy[k]] = true;
        ...
        while (!q.empty()) {
                ...
                visb[p.fi][p.se] = true;
                for (int k = 0; k < 4; k++) {
                        if (0 <= p.fi + dx[k] && p.fi + dx[k] < m && 0 <= p.se + dy[k] && p.se + dy[k] < n) if (v[p.fi + dx[k]][p.se + dy[k]] == '.' && !visb[p.fi + dx[k]][p.se + dy[k]]) {
                                resb++;
                                visb[p.fi + dx[k]][p.se + dy[k]] = true;
                                q.push({p.fi + dx[k], p.se + dy[k]});
                        }
                }
        }
