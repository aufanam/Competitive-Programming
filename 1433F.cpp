...
int digitmul(int a)                                                     {int b=1;while(a>0){b*=a%10;a/=10;}return b;}
                ...
                for (int j = 0; j < m; j++) {
                        for (int cnt = 0; cnt < m/2 + 1; cnt++) {
                                for (int rem = 0; rem < k; rem++) {
                                        ...
                                        if (cnt + 1 <= m/2) {
                                                int nrem = (rem + v[i][j])%k;
                                                if (i != ni) {
                                                        dp[ni][nj][0][nrem] = max(dp[ni][nj][0][nrem], dp[i][j][cnt][rem] + v[i][j]);
                                                } else {
                                                        dp[ni][nj][cnt+1][nrem] = max(dp[ni][nj][cnt+1][nrem], dp[i][j][cnt][rem] + v[i][j]);
                                                }
                                        }
                                }
                        }
