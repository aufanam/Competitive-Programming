...
void emyu() {
        vector<int> tmp;
        for (int i = 0; i < zz.size(); i+=4) {
                int a = ask(zz[i], zz[i+2]);
                if (a == 1) {
                        int b = ask(zz[i], zz[i+3]);
                        if (b == 1) tmp.push_back(zz[i]);
                        else tmp.push_back(zz[i+3]);
                } else if (a == 2) {
                        int b = ask(zz[i+1], zz[i+2]);
                        if (b == 1) tmp.push_back(zz[i+1]);
                        else tmp.push_back(zz[i+2]);
                } else {
                        int b = ask(zz[i+1], zz[i+3]);
                        if (b == 1) tmp.push_back(zz[i+1]);
                        else tmp.push_back(zz[i+3]);
                }
        }
...
void solve() {
        ...
        cin >> zzz;
        int n = pow(2, zzz);
        for (int i = 1; i <= n; i++) zz.push_back(i);
        while (zz.size() >= 4) {
                emyu();
        }
        if (zz.size() == 2) {
