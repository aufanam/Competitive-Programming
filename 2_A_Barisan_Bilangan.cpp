...
void solve() {
        ...
        vector<vector<pair<pair<int, int>, pair<int, int>>>> dp(n+5, vector<pair<pair<int, int>, pair<int, int>>>(k + 5, {{INF, INF}, {INF, 0}}));
        dp[1][0] = {{0, v[1]}, {v[1], v[1]}};
        for (int i = 1; i < n; i++) {
                for (int j = 0; j <= k; j++) {
                        if (dp[i][j].fi.fi == INF) continue;
                        if (j < k) {
                                if (max(dp[i][j].se.se, v[i+1] + dp[i][j].fi.se) - min(dp[i][j].se.fi, v[i+1] + dp[i][j].fi.se) < dp[i+1][j+1].fi.fi) {
                                        dp[i+1][j+1] = {{max(dp[i][j].se.se, v[i+1] + dp[i][j].fi.se) - min(dp[i][j].se.fi, v[i+1] + dp[i][j].fi.se), v[i+1] + dp[i][j].fi.se}, {min(dp[i][j].se.fi, v[i+1] + dp[i][j].fi.se), max(dp[i][j].se.se, v[i+1] + dp[i][j].fi.se)}};
                                }
                        }
                        if (max(dp[i][j].se.se, v[i+1]) - min(dp[i][j].se.fi, v[i+1]) < dp[i+1][j].fi.fi) {
                                dp[i+1][j] = {{max(dp[i][j].se.se, v[i+1]) - min(dp[i][j].se.fi, v[i+1]), v[i+1]}, {min(dp[i][j].se.fi, v[i+1]), max(dp[i][j].se.se, v[i+1])}};
                        }
                }
        }
        int ans = INF;
