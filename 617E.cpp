...
int n, m, k;
int ln = 317; // ceil(sqrt(100.000))
vector<int> cnt(1ll << 20, 0);
int res = 0;
struct Query {
        int l, r, idx;
...
void solve() {
        ...
        vector<vector<Query>> q(n/ln + 3, vector<Query>());
        for (int i = 1; i <= m; i++) {
                int l, r;
                cin >> l >> r;
                l--;
                r++;
                q[l/ln].push_back(Query{l, r, i});
        }
        for (auto &x : q) sort(all(x));
        vector<int> ans(m+5);
        for (int i = 0; i < q.size(); i++) {
                int l, r;
                l = r = i*ln;
                for (auto &z : q[i]) {
                        while (r < z.r) add(pref[r++]);
                        while (l < z.l) del(pref[l++]);
                        while (l > z.l) add(pref[--l]);
                        ans[z.idx] = res;
                }
                for (int j = l; j < r; j++) {
                        del(pref[j]);
