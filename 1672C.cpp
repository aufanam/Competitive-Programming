...
int main()
{
    ...
    while (t--)
    {
        ...
        deque<ll> dq;
        set<ll> booset;
        bool bully = false, bullx = false;
        for (ll i = 2; i <= n; i++)
        {
            cin >> a;
            if (a == cur) dq.push_back(i), booset.insert(a);
            else if (booset.size() == 1) bully = true;
            if (a == cur && bully) bullx = true;
            cur = a;
        }
        if (dq.size() <= 1) cout << "0\n";
        else cout << max(1ll, dq.back() - dq.front() - 1) << '\n';
    }
    // code
    return 0;
}