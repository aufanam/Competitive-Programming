...
void solve() {
        ...
        int n = k*s.length() + 1;
        vector<pair<int, int>> pre(s.length() + 5, {0, 0});
        s = '?' + s;
        ...
        while (q--) {
                ...
                if (range(l, colmx, r)) cout << n << '\n';
                        ...
                        while (lf <= rg) {
                                int md = (lf + rg)/2;
                                int res = (md/(s.length() - 1))*cntb + pre[md%(s.length() - 1)].se + 1; // block ke md dr bawah brd di kolom res
                                if (res > r) {
                                        rg = md - 1;
                                } else if (res < r) {
                                        lf = md + 1;
                        ...
                        while (lf <= rg) {
                                int md = (lf + rg)/2;
                                int res = n - (md/(s.length() - 1))*cnta - pre[md%(s.length() - 1)].fi;
                                // debug(md)
                                // debug(res)
                                if (res > l) {
                                        lf = md + 1;
