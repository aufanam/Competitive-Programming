...
void solve() {
    ...
    vector<int> exist;
    for (int i = 0; i < n; i++) {
        if (mapan.find(v[i]) != mapan.end()) mapan[v[i]]++;
        else exist.push_back(v[i]), mapan.insert(mp(v[i], 1));
    }
    vector<int> pass;
    for (int i = 0; i < exist.size(); i++) {
        if (mapan[exist[i]] >= k) pass.push_back(exist[i]);
    }
    if (pass.size() == 0) {
        cout << "-1\n";
        return;
    ...
    int num = pass[0];
    int cnt = 1;
    int l = pass[0], r = pass[0];
    for (int i = 1; i < pass.size(); i++) {
        if (pass[i] == pass[i-1] + 1) cnt++;
        else {
            if (cnt > ans) {
                ans = cnt;
                l = num;
