...
int zz;
vector<int> v(11111);
vector<int> pre(11111);
bool ok = false;
stack<int> stk;
void check(int x, int cnt, int d) {
        if (x > n) return;
        if (x == n && d == m) {
                int z = pre[x];
                while (pre[z] != -1) {
        ...
        int fd = v[x + zz];
        if (zx != -1) {
                int cc = lower_bound(v.begin() + 1, v.begin() + n + 1, zx + 1) - v.begin() - cnt - 1;
                if (cc >= z && cc <= zz) {
                        pre[x+cc] = x;
                        check(x + cc, cnt + cc, d + 1);
                }
        }
        if (fd != -1) {
                int cc = lower_bound(v.begin() + 1, v.begin() + n + 1, fd + 1) - v.begin() - cnt - 1;
                if (cc >= z && cc <= zz) {
                        pre[x+cc] = x;
                        check(x + cc, cnt + cc, d + 1);
                }
