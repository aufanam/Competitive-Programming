...
void build(int idx, int st, int en) {
        ...
        build(l, st, m);
        build(r, m + 1, en);
        maxtree[idx] = max(maxtree[l], maxtree[r]);
        return;
}
void ud(int idu, int idx, int st, int en) {
        if (st > idu || en < idu) return;
        if (st == idu && en == idu) {
                maxtree[idx] = zx[st];
                return;
        }
...
int qr(int l, int r, int idx, int st, int en) {
        ...
        int rg = lf + 1;
        int m = st + (en - st)/2;
        return max(qr(l, r, lf, st, m), qr(l, r, rg, m + 1, en));
}
void solve() {
        int n, q;
        cin >> n >> q;
