...
void solve() {
        ld s, f, n, d;
        cin >> s >> f >> n >> d;
        vector<pair<double, double>> zz;
        vector<pair<double, double>> v;
        vector<ld> h(n);
        zz.push_back({s + d/(ld)2, 0});
        for (ld i = 0; i < n; i++) {
                ld a, b;
                cin >> a >> b;
                zz.push_back({a, b});
        }
        zz.push_back({f - d/(ld)2, 0});
        ...
        for (auto z : zz) {
                if (stk.size() < 2) {
                        ...
                        pair<ld, ld> zx = stk.top();
                        stk.pop();
                        pair<ld, ld> fd = stk.top();
                        while (orientation({fd.fi, fd.se}, {zx.fi, zx.se}, {z.fi, z.se})) {
                                stk.pop();
                                zx = fd;
                                if (stk.size() > 0) fd = stk.top();
                        }
                        stk.push(zx);
                        stk.push(z);
                }
        ...
        while (stk.size() > 1) {
                ...
                stk.pop();
                pair<ld, ld> fd = stk.top();
                ans += sqrt((zx.fi - fd.fi)*(zx.fi - fd.fi) + (zx.se - fd.se)*(zx.se - fd.se));
        }
