...
vector<vector<pair<int, int>>> par(333, vector<pair<int, int>>(333));
pair<int, int> find(int x, int y) {
        if (par[x][y].fi == -1 && par[x][y].fi == -1) return par[x][y] = mp(x, y);
        if (par[x][y] == mp(x, y)) return mp(x, y);
        return par[x][y] = find(par[x][y].fi, par[x][y].se);
}
void join(pair<int, int> x, pair<int, int> y) {
        pair<int, int> rx = find(x.fi, x.se);
        pair<int, int> ry = find(y.fi, y.se);
        par[rx.fi][rx.se] = ry;
}
void solve() {
        int n, m;
        ...
        map<int, vector<pair<int, int>>> mapan;
        set<int> buset;
        for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                        cin >> v[i][j];
                        par[i][j] = {-1, -1};
                        buset.insert(v[i][j]);
                        mapan[v[i][j]].push_back({i, j});
                }
        }
        vector<int> num;
        for (auto z : buset) num.push_back(z);
        reverse(all(num));
        int ans = 0;
        int dx[] = {1, 0, -1, 0};
        int dy[] = {0, 1, 0, -1};
        for (auto z : num) {
                int cnt = 0;
                for (auto zz : mapan[z]) {
                        set<pair<int, int>> setdah;
                        for (int k = 0; k < 4; k++) {
                                if (range(1, zz.fi + dx[k], n) && range(1, zz.se + dy[k], m) && v[zz.fi+dx[k]][zz.se+dy[k]] >= z) {
                                        setdah.insert(mp(par[zz.fi + dx[k]][zz.se + dy[k]].fi))
                                        join(mp(zz.fi, zz.se), mp(zz.fi + dx[k], zz.se + dy[k]));
                                }
                        }
                        if (par[zz.fi][zz.se].fi == -1 && par[zz.fi][zz.se] == -1) cnt++;
                }
                ckmax(ans, cnt);
        }
        cout << ans << '\n';
