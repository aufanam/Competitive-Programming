...
int n;
vector<vector<int>> v(555, vector<int>(555));
vector<int> color(555);
vector<int> vis(555);
vector<vector<int>> adj(555);
...
bool coba(int x) {
        for (int i = 1; i <= n; i++) color[i] = 0, vis[i] = false, adj[i].clear();
        ...
        for (auto [x, y] : edg) adj[x].push_back(y), adj[y].push_back(x);
        for (int i = 1; i <= 500; i++) if (!vis[i]) dfs(i, 1);
        for (auto [x, y] : edg) if (color[x] == color[y]) return false;
        return true;
}
void solve() {
        cin >> n;
