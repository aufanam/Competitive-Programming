...
pair<int, int> getIntersection(pair<int, int> p1, pair<int, int> p2) {
        int x = max(p1.fi + p1.se, p2.fi + p2.se);
        int y = max(p1.se - p1.fi, p2.se - p2.fi);
        return {(x - y)/2, (x + y)/2};
}
void solve() {
        int n, m;
        ...
        for (auto z : dif) {
                if (z.fi != lst) {
                        ...
                        lst = z.fi;
                        if (zx > m) {
                                p = getIntersection(p, {z.fi, zx - m});
                        }
                }
                fd += z.se;
        }
        for (int i = 1; i <= n; i++) {
                if (getIntersection(p, {x[i], in[i]}) == mp(x[i], in[i])) {
                        cout << "1";
                } else {
                        cout << "0";
                }
