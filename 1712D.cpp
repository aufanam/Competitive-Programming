...
void solve() {
        ...
        // int z = -1;
        // for (int i = 0; i < n - 1; i++) {
        //         ckmax(z, min(v[i], v[i+1]));
        // }
        // int ans = min(z, 2*(*min_element(all(v))));
        // cout << ans << '\n';
        if (k >= 2) {
                for (int i = 0; i < k - 1; i++) {
                        v[zz[i].se] = INF;
                ...
                bool bull = false;
                for (int i = 0; i < n - 1; i++) {
                        if (min(v[i], v[i+1]) == INF) bull = true;
                }
                if (bull) {
                        v[zz[k-1].se] = INF;
                        int ans = min(INF, 2*(*min_element(all(v))));
                        cout << ans << '\n';
                } else {
                        int idx;
                        int mn = INFF;
                        ...
                        ans = max(ans, min(z, 2*(*min_element(all(v)))));
                        cout << ans << '\n';
                }
        } else {
                bool bull = false;
