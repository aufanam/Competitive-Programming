...
int n, m, k;
vector<vector<pair<int, int>>> v(111111);
vector<multiset<int>> dist(111111);
void dijkstra() {
        dist[1].insert(0);
        ...
        for (;!pq.empty();pq.pop()) {
                auto p = pq.top();
                auto it = dist[p.se].begin();
                for (int i = 0; i < k - 1; i++) it++;
                if (p.fi > *it) {
                        continue;
                }
                for (auto z : v[p.se]) {
                        auto itr = dist[z.fi].begin();
                        for (int i = 0; i < k - 1; i++) itr++;
                        if (p.fi + z.se < *itr) {
                                dist[z.fi].insert(p.fi + z.se);
                                pq.push({p.fi + z.se, z.fi});
                        }
                }
        }
        int cnt = 0;
        for (auto it = dist[n].begin();;it++) {
                cout << *it << ' ';
                cnt++;
                if (cnt == k) {
                        return;
