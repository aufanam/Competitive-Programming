...
int main()
{
    ...
    while (t--)
    {
        int n, m, ar, br, ac, bc;
        cin >> n >> m >> ar >> br >> ac >> bc;
        if (ar > ac && br > bc) cout << min(2*n - (ar + ac), 2*m - (br + bc)) << '\n';
        else if (ac >= ar && bc >= br) cout << min(ac - ar, bc - br) << '\n';
        else if (ac >= ar && br > bc) cout << min(ac - ar, 2*m - (br+bc)) << '\n';
        else cout << min(2*n - (ac + ar), bc - br) << '\n';
