...
void solve() {
        ...
        vector<vector<vector<bool>>> vis(n+5, vector<vector<bool>>(m+5, vector<bool>(n*m + 5, false)));
        ...
        while (!q.empty()) {
                ...
                if (p.se.fi == xb && p.se.se == yb) {
                        zz.push_back(p.fi);
                }
                q.pop();
                if (p.fi > n*m) continue;
                for (int k = 0; k < 4; k++) {
                        if (range(1, p.se.fi + dx[k], n) && range(1, p.se.se + dy[k], m) && v[p.se.fi + dx[k]][p.se.se + dy[k]] == '.' && !vis[p.se.fi + dx[k]][p.se.se + dy[k]][p.fi + 1]) {
                                q.push({p.fi + 1, {p.se.fi + dx[k], p.se.se + dy[k]}});
                                vis[p.se.fi + dx[k]][p.se.se + dy[k]][p.fi + 1] = true;
                        }
                }
        }
        sort(all(zz));
