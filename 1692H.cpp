...
void solve() {
        ...
        for (int i = 0; i < ada.size(); i++) {
                vector<int> vv(mapan[ada[i]].size() + 5);
                vv[0] = 1;
                int z = 1;
                for (int j = 1; j < mapan[ada[i]].size(); j++) {
                        z += mapan[ada[i]][j-1] - mapan[ada[i]][j] + 2;
                        vv[j] = z;
                }
                int idxmn = mapan[ada[i]][0];
                int mn = 1;
                // cout << mapan[ada[i]][0] << ' ' << mapan[ada[i]][1] << '\n';
                // print(vv);
                for (int j = 0; j < mapan[ada[i]].size(); j++) {
                        if (vv[j] - mn + 1 > mult) {
                                mult = vv[j] - mn + 1;
                                a = ada[i];
                                lf = idxmn;
