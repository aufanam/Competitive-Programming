...
void solve() {
        ...
        vector<pair<pair<int, int>, pair<int, int>>> rng;
        for (auto z : grd) {
                int imin = z.se;
                int imax = z.se;
                for (int i = z.se; i >= 0; i--) {
                        if (v[z.fi][i] != '#') imin = i;
                        else break;
                }
                for (int i = z.se; i < n; i++) {
                        if (v[z.fi][i] != '#') imax = i;
                        else break;
                }
                rng.push_back({{z.fi, z.se}, {imin, imax}});
        }
        queue<pair<pair<int, int>, int>> q;
        vector<vector<vector<bool>>> vis(n, vector<vector<bool>>(n, vector<bool>(20)));
        q.push({{ax, ay}, 0});
        vis[ax][ay][0] = true;
        int dx[] = {1, 0, -1, 0};
        int dy[] = {0, 1, 0, -1};
        ...
        while (!q.empty()) {
                ...
                if (p.fi.fi == px && p.fi.se == py) {
                        ckmin(ans, p.se);
                }
                q.pop();
                for (int k = 0; k < 4; k++) {
                        if (0 <= p.fi.fi + dx[k] && p.fi.fi + dx[k] < n && 0 <= p.fi.se + dy[k] && p.fi.se + dy[k] < n && v[p.fi.fi + dx[k]][p.fi.se + dy[k]] != '#') {
                                bool ketemu = false;
                                bool tabrak = false;
                                bool jalan = true;
                                for (auto z : rng) {
                                        ...
                                        int lstx;
                                        int lsty;
                                        for (int i = 0; i < p.se + 1; i++) {
                                                lstx = cx;
                                                lsty = cy;
                                                // if (p.fi.fi == 1 && p.fi.se == 0) {
                                                //         debug(cx)
                                                //         debug(cy)
                                                //         debug(v[0][4])
                                                //         debug(d)
                                                // }
                                                if (d == 0) {
                                                        if (cy + 1 < n && v[cx][cy + 1] != '#') cy++;
                                                        else if (cy - 1 >= 0 && v[cx][cy - 1] != '#') cy--, d = 1;
                                                } else {
                                                        if (cy - 1 >= 0 && v[cx][cy - 1] != '#') cy--;
                                                        else if (cy + 1 < n && v[cx][cy + 1] != '#') cy++, d = 0;
                                                }
                                        }
                                        if (cx == p.fi.fi + dx[k] && z.se.fi == z.se.se) jalan = false;
                                        if (cx == p.fi.fi + dx[k] && cy == p.fi.se + dy[k]) ketemu = true;
                                        if (lstx == p.fi.fi + dx[k] && lsty == p.fi.se + dy[k] && cx == p.fi.fi && cy == p.fi.se) tabrak = true;
                                }
                                if (!vis[p.fi.fi + dx[k]][p.fi.se + dy[k]][(p.se + 1)%(2*n)] && !ketemu && !tabrak) {
                                        vis[p.fi.fi + dx[k]][p.fi.se + dy[k]][(p.se + 1)%(2*n)] = true;
                                        q.push({{p.fi.fi + dx[k], p.fi.se + dy[k]}, p.se + 1});
                                }

                        }
                }
                if (v[p.fi.fi][p.fi.se] != '#' && !vis[p.fi.fi][p.fi.se][(p.se + 1)%(2*n)]) {
                        bool ketemu = false;
                        for (auto z : rng) {
                                int cx = z.fi.fi;
                                int cy = z.fi.se;
                                ...
                                if (cx == p.fi.fi && cy == p.fi.se) ketemu = true;
                        }
                        if (!ketemu) {
                                vis[p.fi.fi][p.fi.se][(p.se + 1)%(2*n)] = true;
                                q.push({{p.fi.fi, p.fi.se}, p.se + 1});
                        }
                }
        }
        if (ans != INF) cout << ans << '\n';
