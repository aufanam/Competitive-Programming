...
void solve() {
        int n, m;
        cin >> n >> m;
        vector<vector<int>> aw(n+5, vector<int>(m+5, 0));
        vector<vector<int>> pre(n+5, vector<int>(m+5, 0));
        for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                        int x;
                        cin >> x;
