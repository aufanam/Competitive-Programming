...
vector<int> tpref(N);
vector<int> tsuf(N);
void buildmax(int idx, int st, int en) {
        if (st == en) {
                tmax[idx] = st;
                return;
        }
        ...
        buildmax(l, st, m);
        buildmax(r, m + 1, en);
        tmax[idx] = (zx[tmax[l]] > zx[tmax[r]] ? tmax[l] : tmax[r]);
        return;
}
int querymax(int l, int r, int idx, int st, int en) {
        ...
        int rg = lf + 1;
        int m = (st + en)/2;
        int p1 = querymax(l, r, lf, st, m);
        int p2 = querymax(l, r, rg, m + 1, en);
        return (zx[p1] > zx[p2] ? p1 : p2);
}
void buildpref(int idx, int st, int en) {
        if (st == en) {
                tpref[idx] = st;
                return;
        }
        ...
        buildpref(l, st, m);
        buildpref(r, m + 1, en);
        tpref[idx] = (pref[tpref[l]] > pref[tpref[r]] ? tpref[l] : tpref[r]);
        return;
}
int querypref(int l, int r, int idx, int st, int en) {
        if (st > r || en < l) return 0;
        if (st >= l && en <= r) {
                return tpref[idx];
        }
        ...
        int rg = lf + 1;
        int m = (st + en)/2;
        int p1 = querypref(l, r, lf, st, m);
        int p2 = querypref(l, r, rg, m + 1, en);
        return (pref[p1] > pref[p2] ? p1 : p2);
}
void buildsuf(int idx, int st, int en) {
        if (st == en) {
                tsuf[idx] = st;
                return;
        }
        ...
        buildsuf(l, st, m);
        buildsuf(r, m + 1, en);
        tsuf[idx] = (suf[tsuf[l]] > suf[tsuf[r]] ? tsuf[l] : tsuf[r]);
        return;
}
int querysuf(int l, int r, int idx, int st, int en) {
        if (st > r || en < l) return 0;
        if (st >= l && en <= r) {
                return tsuf[idx];
        }
        ...
        int rg = lf + 1;
        int m = (st + en)/2;
        int p1 = querysuf(l, r, lf, st, m);
        int p2 = querysuf(l, r, rg, m + 1, en);
        return (suf[p1] > suf[p2] ? p1 : p2);
}
void solve() {
        cin >> n;
        for (int i = 1; i <= n; i++) {
        ...
        for (int i = 1; i <= n; i++) {
                ...
                int kpref = querypref(i, idxr - 1, 1, 1, n);
                int ksuf = querysuf(idxl + 1, i, 1, 1, n);
                int sum = pref[kpref] - pref[ksuf - 1];
                if (zx[i] < sum) {
                        cout << "NO\n";
                        debug(i)
                        return;
