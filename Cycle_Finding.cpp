...
int n, m;
vector<int> dist(5555);
vector<pair<pair<int, int>, int>> v;
vector<bool> valid(5555, false);
vector<bool> vis(5555, false);
vector<vector<int>> adj(5555);
vector<int> pref(5555, -1);
void emyu() {
        for (int i = 1; i <= n; i++) {
                dist[i] = 0;
        }
        ...
        for (int i = 0; i <= n; i++) {
                for (auto z : v) {
                        if (dist[z.fi.fi] + z.se < dist[z.fi.se]) {
                                if (i == n) {
                                        pref[z.fi.se] = z.fi.fi;
                                }
                                dist[z.fi.se] = dist[z.fi.fi] + z.se;
                        }
                }
        }
        for (int i = 1; i <= n; i++) {
