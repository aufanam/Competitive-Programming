...
void solve() {
        ...
        for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                        ...
                        ld cc = v[j].fi.fi;
                        ld dd = v[j].fi.se;
                        ld zz = abs(aa-cc)*abs(aa-cc) + abs(bb-dd)*abs(bb-dd);
                        ld xx = v[i].se + v[j].se;
                        if (xx*xx >= zz && zz >= abs(v[i].se - v[j].se)*abs(v[i].se - v[j].se)) {
                                join(i, j);
                        }
                }
        }
        ...
        for (int i = 0; i < n; i++) {
                ...
                ld bb = v[i].fi.se;
                ld cc = v[i].se;
                ld zz = abs(aa-sx)*abs(aa-sx) + abs(bb-sy)*abs(bb-sy);
                ld xx = abs(aa-tx)*abs(aa-tx) + abs(bb-ty)*abs(bb-ty);
                if (cc*cc == zz) {
                        idxs = i;
                }
                if (cc*cc == xx) {
