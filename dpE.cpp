...
int n;
vector<int> v(111);
vector<vector<int>> dp1(111, vector<int>(111));
vector<vector<int>> dp2(111, vector<int>(111));
vector<int> dp3(111);
const int modd = 26101991;
int rec1(int l, int r) {
        if (dp1[l][r] != -1) return dp1[l][r];
        if (r - l <= 1) return dp1[l][r] = 0;
        int res = 8*INFF;
        for (int i = l + 1; i < r; i++) {
                ckmin(res, rec1(l, i) + rec1(i, r) + v[l]*v[i]*v[r]);
        }
        return dp1[l][r] = res;
}
int rec2(int l, int r) {
        if (dp2[l][r] != -1) return dp2[l][r];
        if (r - l <= 1) return dp2[l][r] = 1;
        int zx = rec1(l, r);
        int res = 0;
        for (int i = l + 1; i < r; i++) {
                int fd = rec1(l, i) + rec1(i, r) + v[l]*v[i]*v[r];
                if (fd == zx) {
                        res += ((rec2(l, i)%modd)*(rec2(i, r)%modd))%modd;
                }
        }
        return dp2[l][r] = res;
}
int rec3() {
        ...
        for (int i = 2; i < n; i++) {
                for (int j = 0; j < i; j++) {
                        dp3[i] = (dp3[i] + dp3[j]*dp3[i-j-1])%modd;
                }
        }
        return dp3[n-1];
}
