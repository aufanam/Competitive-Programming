...
int n, m;
vector<vector<int>> v(40, vector<int>(40));
vector<int> pos;
int dx[] = {1, 0, -1, 0};
int dy[] = {0, 1, 0, -1};
...
void rec(int x, int y, int cnt) {
        ...
        vis[x][y] = true;
        for (int k = 0; k < 4; k++) {
                if (range(1, x + dx[k], n) && range(1, y + dy[k], m) && !vis[x+dx[k]][y+dy[k]]) {
                        rec(x + dx[k], y + dy[k], cnt);
                }
        }
        vis[x][y] = false;
}
void solve() {
        ...
        while (q--) {
                int l, r;
                cin >> l >> r;
                cout << upper_bound(all(pos), r) - lower_bound(all(pos), l) << '\n';
        }
        return;
}
