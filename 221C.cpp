...
void solve() {
        ...
        if (cnt == 2) {
                swap(v[idx[0]-1], v[idx[1]]);
                if (is_sorted(all(v))) {
                        cout << "YES\n";
                        return;
                }
                swap(v[idx[0]-1], v[idx[1]]);
                swap(v[idx[0]], v[idx[1]]);
                if (is_sorted(all(v))) {
                        cout << "YES\n";
                        return;
                }
                swap(v[idx[0]], v[idx[1]]);
                swap(v[idx[0]-1], v[idx[1]-1]);
                if (is_sorted(all(v))) {
                        cout << "YES\n";
                        return;
                }
                swap(v[idx[0]-1], v[idx[1]-1]);
                swap(v[idx[0]], v[idx[1]-1]);
                if (is_sorted(all(v))) {
                        cout << "YES\n";
                        return;
                }
                ...
                swap(v[id], v[idx[0]]);
                for (int i = 0; i < n; i++) {
                        if (v[i] >= v[idx[0]-1] && i != idx[0] - 1) {
                                if (i > 0) {
                                        swap(v[i-1], v[idx[0]-1]);
                                }
                                break;
                        }
                        if (i == n - 1) {
                                swap(v[n-1], v[idx[0]-1]);
                        }
                }
                if (is_sorted(all(v))) {
                        cout << "YES\n";
