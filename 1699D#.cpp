...
void solve() {
        ...
        for (int i = 1; i <= n; i++) {
                ...
                if (v[0] != i) bb.push_back(mp(0, i));
                for (int j = 0; j < n; j++) {
                        if (v[j] != i) {
                                if (cntcr > 0) bb.push_back(mp(cntcr, i));
                                mapan[v[j]]++;
                                cntcur++;
                                ok = true;
                        } else {
                ...
                ckmax(ans, cnt - res);
                int rem = n - zx;
                for (int i = 1; i < bb.size(); i+=2) {
                        if (bb[i].fi == -1 && bb[i].se == -1) continue;
                        if (bb[i].fi >= bb[i+1].fi) {
                                bb[i].fi -= bb[i+1].fi;
                                cnt -= bb[i+1].fi;
                                rem -= bb[i+1].fi;
                                if (i < bb.size() - 2) {
                                        if (bb[i].se != bb[i+2].se) {
                                                rem -= min(bb[i].fi, bb[i+2].fi);
                                                if (bb[i].fi > bb[i+2].fi) {
                                                        bb[i+2] = {bb[i].fi - bb[i+2].fi, bb[i].se};
                                                } else if (bb[i].fi < bb[i+2].fi) {
                                                        bb[i+2] = {bb[i+2].fi - bb[i].fi, bb[i+2].se};
                                                } else {
                                                        bb[i+2] = {-1, -1};
                                                }
                                        } else {
                                                bb[i+2] = {bb[i].fi + bb[i+2].fi, bb[i+2].se};
                                        }
                                }
                        }
                }
