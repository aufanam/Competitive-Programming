...
char number(ll n)                                                       {string bruh="0123456789"; return bruh[n];}
...
void solve() {
        string s;
        cin >> s;
        vector<set<int>> setpos(26);
        for (int i = 0; i < s.length(); i++) {
                setpos[s[i] - 'a'].insert(i);
        }
        int q;
        cin >> q;
        while (q--) {
                ...
                if (t == 1) {
                        ...
                        cin >> pos >> c;
                        pos--;
                        setpos[s[pos] - 'a'].erase(pos);
                        s[pos] = c;
                        setpos[s[pos] - 'a'].insert(pos);
                } else {
                        int l, r;
                        cin >> l >> r;
                        l--;
                        ...
                        int cnt = 0;
                        for (int i = 0; i < 26; i++) {
                                auto it = setpos[i].lower_bound(l);
                                if (it != setpos[i].end() && *it <= r) cnt++;
                        }
                        cout << cnt << '\n';
                }
        }
