...
int n, m;
vector<vector<char>> v(111, vector<char>(111));
bool find(string fin) {
        vector<bool> b(10);
        for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                        if (v[i][j] == fin[0]) {
                                for (int k = 1; k <= 8; k++) b[k] = true;
                                for (int k = 1; k < fin.length(); k++) {
                                        if (j + k <= m) {if (v[i][j+k] != fin[k]) b[1] = false;}
                                        else b[1] = false;
                                        if (j - k >= 1) {if (v[i][j-k] != fin[k]) b[2] = false;}
                                        else b[2] = false;
                                        if (i + k <= n) {if (v[i+k][j] != fin[k]) b[3] = false;}
                                        else b[3] = false;
                                        if (i - k >= 1) {if (v[i-k][j] != fin[k]) b[4] = false;}
                                        else b[4] = false;
                                        if (i + k <= n && j + k <= m) {if (v[i+k][j+k] != fin[k]) b[5] = false;}
                                        else b[5] = false;
                                        if (i + k <= n && j - k >= 1) {if (v[i+k][j-k] != fin[k]) b[6] = false;}
                                        else b[6] = false;
                                        if (i - k >= 1 && j + k <= m) {if (v[i-k][j+k] != fin[k]) b[7] = false;}
                                        else b[7] = false;
                                        if (i - k >= 1 && j - k >= 1) {if (v[i-k][j-k] != fin[k]) b[8] = false;}
                                        else b[8] = false;
                                }
                                for (int k = 1; k <= 8; k++) if (b[k]) return true;
                        }
                }
        }
        return false;
