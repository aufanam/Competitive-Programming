...
void solve() {
        ...
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                        if (v[i][j] == 'B') {
                                if (a.fi == -1 || i + j > a.fi + a.se) a = {i, j};
                                if (b.fi == -1 || i + j < b.fi + b.se) b = {i, j};
                                if (c.fi == -1 || i - j > c.fi - c.se) c = {i, j};
                                if (d.fi == -1 || i - j < d.fi - d.se) d = {i, j};
                        }
                }
        }
        edge.push_back(a);
        ...
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                        int dist = 0;
                        for (auto it : edge) dist = max(dist, abs(i - it.fi) + abs(j - it.se));
                        if (dist < ans) {
                                ans = dist;
                                p = {i, j};
                        }
