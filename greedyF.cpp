...
void solve() {
        ...
        vector<pair<long long, long long>> pass;
        long long num;
        for (long long i = 0; i <= n; i++) {
                if (v[i] == mn) {
                        num = i;
        ...
        long long mxx = 0;
        long long len = m/mn;
        long long rem = m - len*(v[num]);
        long long cnt = 0;
        // debug(num)
        for (long long j = n; j > num; j--) {
                long long z = v[j] - v[num];
                if (z > rem) continue;
                long long x = rem/z;
                pass.push_back({j, x});
                cnt += x;
        ...
        if (num == 0) {
                ...
                if (pass.size() == 0) {
                        for (long long i = 1; i <= n; i++) {
                                ...
                                long long zx = z - rem;
                                if (zx < 0) continue;
                                long long x = (long long)ceildiv((long long)z - (long long)rem, (long long)v[0]);
                                if (x <= mnn) {
                                        mnn = x;
                                        noom = i;
                                }
                        ...
                        pass.push_back({noom, 1});
                        pass.push_back({num, len - mnn});
                        len = len - mnn + 1;
                } else pass.push_back({num, len - cnt});
        }
        else pass.push_back({num, len - cnt});
        mxx = pass.size();
        if (pass[0].fi == 0) {
                ...
                cout << len << '\n';
                long long p = 0;
                vector<pair<long long, long long>> turu = pass;
                for (long long i = 0; i < min(len, 50ll); i++) {
                        while (turu[p].se == 0) p++;
                        cout << turu[p].fi;
                        turu[p].se--;
                        if (turu[p].se == 0) p++;
                ...
                stack<long long> stk;
                p = mxx - 1;
                for (long long i = len - 1; i >= max(0ll, len - 50ll); i--) {
                        while (pass[p].se == 0) p--;
                        stk.push(pass[p].fi);
                        pass[p].se--;
                        if (pass[p].se == 0) p--;
