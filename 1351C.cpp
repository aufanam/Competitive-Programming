...
void solve() {
        map<pair<pair<int, int>, pair<int, int>>, bool> mapan;
        int x = 0;
        int y = 0;
        string s;
        cin >> s;
        ...
        for (int i = 0; i < s.length(); i++) {
                if (s[i] == 'N') {
                        if (mapan[{{x, y}, {x, y+1}}]) ans++;
                        else ans += 5;
                        mapan[{{x, y}, {x, y+1}}] = true;
                        mapan[{{x, y+1}, {x, y}}] = true;
                        y++;
                }
                if (s[i] == 'S') {
                        if (mapan[{{x, y}, {x, y-1}}]) ans++;
                ...
                if (s[i] == 'W') {
                        if (mapan[{{x, y}, {x-1, y}}]) ans++;
                        else ans += 5;
                        mapan[{{x, y}, {x-1, y}}] = true;
                        mapan[{{x-1, y}, {x, y}}] = true;
                        x--;
                }
                if (s[i] == 'E') {
                        if (mapan[{{x, y}, {x+1, y}}]) ans++;
