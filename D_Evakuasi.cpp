...
void solve() {
        ...
        atk[b] = INFF;
        // for (ld i = 1; i <= n; i++) cout << atk[i] << ' ';
        // cout << '\n';
        vector<vector<pair<ld, ld>>> adj(n+5);
        for (ld i = 0; i < m; i++) {
                ld c, d, l;
                cin >> c >> d >> l;
                adj[c].push_back({d, l});
        ...
        while (!pq.empty()) {
                ...
                pq.pop();
                for (auto z : adj[p.se]) {
                        if (p.fi + z.se/v < dist[z.fi] && p.fi + z.se/v < atk[z.fi]) {
                                // cout << p.fi + z.se/v << '\n';
                                dist[z.fi] = p.fi + z.se/v;
                                pq.push({dist[z.fi], z.fi});
                        }
                }
        }
        if (dist[b] == INFF) cout << "GAME OVER\n";
        else cout << dist[b] << '\n';
        return;
}
