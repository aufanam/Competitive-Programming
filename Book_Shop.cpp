...
int n, x;
vector<int> price(1111); // price of each book
vector<int> page(1111); // page of each book
int ans = 0;
void rec(int currentCost, int currentPage, int currentIndex) {
        ckmax(ans, currentPage);
        if (currentIndex > n) return;                                                                                                                                               ;
        for (int i = currentIndex; i <= n; i++) {
                if (currentCost + price[i] <= x) {
                        rec(currentCost + price[i], currentPage + page[i], i + 1);
                }
        }
}
void solve() {
