...
int n, m, s;
int idu; // idx update
int lu, ru; // l, r update
vector<int> dp(N, INFF);
vector<int> tr(N, INFF);
vector<vector<pair<int, int>>> v(N);
void update(int idx, int l, int r) {
        if (l > idu || r < idu) return;
        if (l == idu && r == idu) {
                tr[idx] = dp[l];
                return;
        }
        ...
        update(lf, l, md);
        update(rg, md + 1, r);
        tr[idx] = min(tr[lf], tr[rg]);
}
int query(int idx, int l, int r) {
        if (l > ru || r < lu) return INFF;
        if (l >= lu && r <= ru) return tr[idx];
        int lf = 2*idx;
        int rg = lf + 1;
        int md = (l + r)/2;
        return min(query(lf, l, md), query(rg, md + 1, r));
}
void zz() {for (int i = 1; i <= n; i++) v[i].push_back({i, s});}
void solve() {
        cin >> n >> m >> s;
        for (int i = 1; i <= m; i++) {
                int a, b, c;
        ...
        for (int i = 1; i <= n; i++) {
                for (int j = 0; j < v[i].size(); j++) {
                        ...
                        ru = i - 1;
                        int res;
                        res = (lu ? query(1, 1, n) : 0);
                        dp[i] = min(dp[i], res + v[i][j].se);
                }
                idu = i;
                update(1, 1, n);
        }
