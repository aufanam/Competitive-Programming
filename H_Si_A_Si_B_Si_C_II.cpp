...
void solve() {
        ...
        int ans = 0;
        int sama3 = x;
        int sama2tanpa1 = y - x - 1;
        int sama2tanpa3 = z - x - 1;
        for (int i = 1; i <= x; i++) {
                int batasz = max(0ll, z - i - 1);
                int bataszx = max(0ll, z - y - 1);
                int res = batasz*(batasz+1)/2 - bataszx*(bataszx+1)/2;
                ans += res*6;
        }
        ans += sama3;
        ans += (y*(y-1)/2 - sama2tanpa1*(sama2tanpa1 + 1)/2)*3;
        ans += (z*(z-1)/2 - sama2tanpa3*(sama2tanpa3 + 1)/2)*3;
        cout << ans << '\n';
        return;
}
