...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            if (s[i-1] == '0') ans.push_back(mp(i-1, i));
            if (s[i] == '0' && s[i-1] == '1') left.push_back(i);
            if (s[i] == '1' && s[i-1] == '1') left.push_back(i);
        }
        if (s[0] == '0')
        {
            if (s[n-1] == '0') ans.push_back(mp(n-1, 0));
            else left.push_back(0);
        }
        if (s[0] == '1' && s[n-1] == '1') left.push_back(0);
        if (s[0] == '1' && s[n-1] == '0') ans.push_back(mp(n-1, 0));
        if (left.size()%2 || left.size() == 0)
        {
            cout << "NO\n";
            continue;
        }
        ...
        for (int i = 1; i < left.size(); i++) cout << left[i] + 1 << ' ' << left[0] + 1 << '\n';
    }
    // code
    return 0;
}