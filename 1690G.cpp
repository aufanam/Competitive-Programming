...
void solve() {
        ...
        buset.insert(mp(0, v[0]));
        for (int i = 1; i < n; i++) {
                if (v[i] < buset.rbegin()->se) {
                        buset.insert(mp(i, v[i]));
                }
        }
        while (m--) {
                int k, d;
                ...
                k--;
                v[k] -= d;
                auto it = buset.upper_bound(mp(k, INF));
                it--;
                if (v[k] < it->se || it->fi == k) {
                        if (it->fi == k) {
                                buset.erase(it);
                        }
                        auto itx = buset.insert(mp(k, v[k])).fi;
                        do {
                                auto itd = itx;
                                itd++;
                                if (itd == buset.end()) {
