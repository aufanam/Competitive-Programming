...
int n, k;
vector<int> v(205, 0);
// vector<vector<vector<int>>> dp(205, vector<vector<int>>(205, vector<int>(1005, -1)));
int dp[205][205][1005];
int rec(int x, int y, int z) {
        ...
        int zz = y*(v[x+1] - v[x]) + z;
        if (zz > k) {
                dp[x][y][z] = 0;
                return 0;
        }
        dp[x][y][z] = (int)((1LL*rec(x+1, y+1, zz) + 1LL*rec(x+1, y, zz)*(y+1) + 1LL*rec(x+1, y-1, zz)*y)%MOD);
        return dp[x][y][z];
}
void solve() {
        cin >> n >> k;
