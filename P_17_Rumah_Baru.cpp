...
bool check(int x, int y) {
        ...
        b1 = b2 = b3 = b4 = false;
        for (int i = 0; i < n; i++) {
                if (v[i].fi == x && v[i].se > y) b1 = true;
                if (v[i].fi == x && v[i].se < y) b2 = true;
                if (v[i].fi > x && v[i].se == y) b3 = true;
                if (v[i].fi < x && v[i].se == y) b4 = true;
        }
        if (b1 and b2 and b3 and b4) return true;
        return false;
}
void solve() {
        cin >> n;
