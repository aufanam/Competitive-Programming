...
void solve() {
        ...
        while (true) {
                ...
                int zx = low.size();
                int fd = high.size();
                for (int i = 0; i < zx/2; i++) {
                        char c = ask(low[i], low[i+zx/2]);
                        if (c == '>') {
                                lo.push_back(low[i+zx/2]);
                        } else {
                                lo.push_back(low[i]);
                        }
                }
                if (zx%2) {
                        lo.push_back(low[zx-1]);
                }
                for (int i = 0; i < fd/2; i++) {
                        char c = ask(high[i], high[i+fd/2]);
                        if (c == '<') {
                                hi.push_back(high[i+fd/2]);
                        } else {
                                hi.push_back(high[i]);
                        }
                }
                if (fd%2) {
                        hi.push_back(high[fd-1]);
                }
        }
        return;
}
