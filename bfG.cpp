...
int n, m;
int cnt = 0;
vector<vector<int>> vv(30, vector<int>(30, 0));
...
void rec(int x, int y, int c) {
        ...
        if (x > 0) if (v[x-1][y] == c) rec(x - 1, y, c);
        if (x < n - 1) if (v[x+1][y] == c) rec(x + 1, y, c);
        if (y > 0) if (v[x][y-1] == c) rec(x, y - 1, c);
        if (y < m - 1) if (v[x][y+1] == c) rec(x, y + 1, c);
}
void rec2(int x, int y, int c) {
        if (vis[{x, y}]) return;
        vis[{x, y}] = true;
        if (vv[x][y] == c) {
                cnt++;
        ...
        if (x > 0) if (vv[x-1][y] == c) rec2(x - 1, y, c);
        if (x < n - 1) if (vv[x+1][y] == c) rec2(x + 1, y, c);
        if (y > 0) if (vv[x][y-1] == c) rec2(x, y - 1, c);
        if (y < m - 1) if (vv[x][y+1] == c) rec2(x, y + 1, c);
}
...
void solve() {
        ...
        for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                        ...
                        for (auto z : zz) {
                                vv[z.fi][z.se] = -1;
                                done1[{z.fi, z.se}] = true;
                        }
                        for (int ki = 0; ki < m; ki++) {
                                for (int kj = n - 2; kj >= 0; kj--) {
                                        if (vv[kj][ki] != -1) {
                                                int x = kj;
                                                int y = ki;
                                                while (vv[x+1][y] == -1 && x < n - 1) {
                                                        swap(vv[x][y], vv[x+1][y]);
                                                        x++;
                                                }
                                        }
                                }
