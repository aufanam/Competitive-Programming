...
void dijkstra(int st) {
        ...
        while (!pq.empty()) {
                ...
                pq.pop();
                for (auto z : v[p.fi]) {
                        if (p.se + z.se < dist[st][z.fi]) {
                                dist[st][z.fi] = p.se + z.se;
                                pq.push({z.fi, dist[st][z.fi]});
                        }
                }
        }
}
void solve() {
        ...
        for (auto z : edg) {
                int res = 0;
                for (auto p : dlv) {
                        res += min({dist[p.fi][p.se], dist[p.fi][z.fi] + dist[z.se][p.se], dist[p.fi][z.se] + dist[z.fi][p.se]});
                }
                ckmin(ans, res);
        }
        cout << ans << '\n';
