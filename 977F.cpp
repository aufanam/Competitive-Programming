...
int rec(int num, int idx) {
        ...
        int mx = 1;
        if (!adagak[num - 1]) goto Next;
        else {
                int l = 0;
                int r = capek[num - 1].size();
                int idxx = -1;
                while (l < r) {
                        int m = (l+r)/2;
                        if (capek[num - 1][m] >= idx) {
                                r = m;
                        } else {
                                idxx = capek[num - 1][m];
                                l = m + 1;
...
void solve() {
        ...
        for (int i = 1; i <= n; i++) {
                cin >> arr[i];
                capek[arr[i]].push_back(i);
                adagak[arr[i]] = true;
        }
        int ans = 1;
        for (int i = n; i >= 1; i--) ckmax(ans, rec(arr[i], i));
        cout << ans << '\n';
        for (int i = n; i >= 1; i--) {
                if (dp[i] == ans) {
                        vector<int> v;
