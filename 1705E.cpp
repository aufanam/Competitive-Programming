...
void solve() {
        ...
        for (int i = 1; i <= n; i++) {
                ...
                cnt[v[i]]++;
                buset.insert(v[i]);
                if (cnt[v[i]] == 1) gada.erase(v[i]);
        }
        for (int i = 1; i <= 200000; i++) {
                if (cnt[i] > 1) setdah.insert(i);
        }
        for (int i = mx; i >= 1; i--) {
                if (cnt[i] > 0) mxx = i;
                else break;
        ...
        while (q--) {
                ...
                cin >> k >> l;
                cnt[v[k]]--;
                if (cnt[v[k]] == 1) setdah.erase(v[k]);
                if (cnt[v[k]] == 0) buset.erase(v[k]), gada.insert(v[k]);
                v[k] = l;
                cnt[v[k]]++;
                if (cnt[v[k]] == 2) setdah.insert(v[k]);
                if (cnt[v[k]] == 1) buset.insert(v[k]), gada.erase(v[k]);
                int res = *buset.rbegin();
                int zx = -1;
                if (!setdah.empty()) zx = *setdah.rbegin();
                auto fd = gada.lower_bound(zx + 1);
                // debug(res)
                // debug(zx)
                // debug(*fd)
                if (zx == -1) {
