...
void solve() {
        ...
        for (int i = 1; i < n; i++) {
                ...
                Next:;
                zx.push_back({posmn, posmx});
        }
        vector<vector<pair<int, int>>> dp(n+5, vector<pair<int, int>>(2));
        dp[0][0] = {zz, zz};
        dp[0][1] = {zz, zz};
        for (int i = 1; i < n; i++) {
                int kk = max(dp[i-1][0].se, zx[i].fi) - min(dp[i-1][0].fi, zx[i].fi);
                int kkk = max(dp[i-1][1].se, zx[i].fi) - min(dp[i-1][1].fi, zx[i].fi);
                int kkkk = max(dp[i-1][0].se, zx[i].se) - min(dp[i-1][0].fi, zx[i].se);
                int kkkkk = max(dp[i-1][1].se, zx[i].se) - min(dp[i-1][1].fi, zx[i].se);
                if (kk < kkk) {
                        dp[i][0] = {min(dp[i-1][0].fi, zx[i].fi), max(dp[i-1][0].se, zx[i].fi)};
                } else {
                        dp[i][0] = {min(dp[i-1][1].fi, zx[i].fi), max(dp[i-1][1].se, zx[i].fi)};
                }
                if (kkkk < kkkkk) {
                        dp[i][1] = {min(dp[i-1][0].fi, zx[i].se), max(dp[i-1][0].se, zx[i].se)};
                } else {
                        dp[i][1] = {min(dp[i-1][1].fi, zx[i].se), max(dp[i-1][1].se, zx[i].se)};
                }
        }
        int ans = min(dp[n-1][0].se - dp[n-1][0].fi, dp[n-1][1].se - dp[n-1][1].fi);
        zz = v[n-1];
        zx.clear();
        zx.push_back({zz, zz});
        for (int i = 0; i < n - 1; i++) {
        ...
        // for (auto xx : zx) cout << xx.fi << ' ' << xx.se << '\n';
        dp[0][0] = {zz, zz};
        dp[0][1] = {zz, zz};
        for (int i = 1; i < n; i++) {
                int kk = max(dp[i-1][0].se, zx[i].fi) - min(dp[i-1][0].fi, zx[i].fi);
        ...
        ckmin(ans, min(dp[n-1][0].se - dp[n-1][0].fi, dp[n-1][1].se - dp[n-1][1].fi));
        cout << ans << '\n';
        return;
}
