...
void callprime()                                                        {for(int i=2;i<=40000;i++){if(!isPrime[i]){int j=i*i;while(j<=40000){isPrime[j]=true;j+=i;}}}isPrime.flip();isPrime[1]=false;}
map<int, int> mapan;
vector<int> primes;
void solve() {
        int a, b, k;
        ...
        if (a > 1 && !mapan.count(aawal)) cnta = max(1ll, cnta + 1);
        if (b > 1 && !mapan.count(bawal)) cntb = max(1ll, cntb + 1);
        if (!mapan.count(aawal)) mapan.insert({aawal, cnta});
        if (!mapan.count(bawal)) mapan.insert({bawal, cntb});
        // cout << cnta << ' ' << cntb << '\n';
        if (aawal == bawal) {
                if (k <= cnta + cntb && k > 1) {
                        cout << "YES\n";
                ...
                if (aawal > bawal && aawal%bawal == 0) {
                        if (k <= cnta + cntb && k > 0) {
                                cout << "YES\n";
                        } else {
                                cout << "NO\n";
                        }
                } else if (bawal > aawal && bawal%aawal == 0) {
                        if (k <= cnta + cntb && k > 0) {
                                cout << "YES\n";
                        } else {
                                cout << "NO\n";
