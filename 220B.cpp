...
void solve() {
        ...
        vector<vector<int>> pref(n+1, vector<int>(zz.size()+2));
        for (int i = 1; i <= n; i++) {
                for (auto z : buset) {
                        pref[i][zz[z]] = pref[i-1][zz[z]];
                }
                if (v[i-1] <= 100000) if (ok[v[i-1]]) pref[i][zz[v[i-1]]]++;
        }
        while (q--) {
                int l, r;
                cin >> l >> r;
                ...
                for (auto z : buset) {
                        if (r - l + 1 < z) break;
                        if (pref[r][zz[z]] - pref[l-1][zz[z]] == z) {
                                cnt++;
                        }
                }
                cout << cnt << '\n';
