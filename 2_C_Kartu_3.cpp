...
void solve() {
        ...
        for (int i = 0; i < n; i++) {
                ...
                bool bull2 = true;
                for (int j = 1; j < n; j++) {
                        if (v[i][j] != v[i][0]) bull1 = false;
                        if (v[j][i] != v[0][i]) bull2 = false;
                }
                if (bull1) mapan[v[i][0]] = true;
                if (bull2) mapan[v[0][i]] = true;
        }
        if (v[0][0] == v[1][1] && v[1][1] == v[2][2]) mapan[v[1][1]] = true;
        if (v[0][2] == v[1][1] && v[1][1] == v[2][0]) mapan[v[1][1]] = true;
        if (mapan['A'] && mapan['B']) {
                cout << "S\n";
                return;
        } else if (mapan['A']) {
