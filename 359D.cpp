...
int n;
vector<int> zx(N);
vector<vector<int>> SparseGcd(333333, vector<int>(20));
void buildsparsegcd() {
        for (int i = 0; i < n; i++) {
                SparseGcd[i][0] = zx[i];
        }
        for (int j = 1; (1ll << j) <= n; j++) {
                for (int i = 0; (i + (1ll << j) - 1) < n; i++) {
                        SparseGcd[i][j] = gcd(SparseGcd[i][j-1], SparseGcd[i + (1ll << (j - 1))][j-1]);
                }
        }
}
int querygcd(int l, int r) {
        int j = (int)log2(r - l + 1);
        return gcd(SparseGcd[l][j], SparseGcd[r - (1 << j) + 1][j]);
}
void solve() {
        cin >> n;
        vector<int> v(n);
