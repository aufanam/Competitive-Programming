...
void solve() {
        ...
        vector<vector<pair<int, int>>> prefr(n+1, vector<pair<int, int>>(m+1, {0, 0}));
        for (int i = 1; i <= n; i++) {
                if (v[i][1] != 9999) prefr[i][1].fi = v[i][1];
                else prefr[i][1].se++;
        }
        for (int i = 1; i <= n; i++) {
                for (int j = 2; j <= m; j++) {
                        prefr[i][j].fi = prefr[i][j-1].fi;
                        prefr[i][j].se = prefr[i][j-1].se;
                        if (v[i][j] != 9999) prefr[i][j].fi += v[i][j];
                        else prefr[i][j].se++;
                }
        }
        vector<vector<pair<int, int>>> pre(n+1, vector<pair<int, int>>(m+1, {0, 0}));
        for (int i = 1; i <= m; i++) {
                pre[1][i].fi = prefr[1][i].fi;
                pre[1][i].se = prefr[1][i].se;
        }
        for (int i = 2; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                        pre[i][j].fi = pre[i-1][j].fi + prefr[i][j].fi;
                        pre[i][j].se = pre[i-1][j].se + prefr[i][j].se;
                }
        }
